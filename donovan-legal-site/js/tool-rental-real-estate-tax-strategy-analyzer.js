...
// =============================================================================
// TIER GATING — single codebase, two render modes.
// The HTML wrapper sets window.__DONOVAN_TIER to 'public' (open URL) or 'client'
// (gold/, platinum/, reserve/ behind cPanel Basic Auth) BEFORE this script loads.
// In 'public' mode, premium panels render their structure with a blur overlay
// and a "Members only" CTA; the underlying calculation engine still runs so the
// eligibility analysis and Year 1 net cash benefit can be shown openly.
// =============================================================================
const TIER = (typeof window !== 'undefined' && window.__DONOVAN_TIER) || 'public';
function isPublic() { return TIER === 'public'; }
function isClient() { return TIER === 'client'; }

// Wrap an HTML fragment in a locked-panel overlay. Caller supplies a short
// teaser title and body; the CTA is firm-standard.
function tierLock(panelHtml, title, body) {
  ...
  return `<div class="tier-locked-panel">
    ...
    <div class="tier-locked-overlay">
      ...
      <a href="engagement.html" class="tier-lock-cta">Learn about membership &rarr;</a>
      <div style="margin-top:0.6rem; font-size:0.72rem; color:#6b6b6b;">Already a member? <a href="engagement.html" style="color:#169B62; text-decoration:underline;">Sign in</a> to unlock.</div>
    </div>
  </div>`;
}
...
// =============================================================================
// LOCALITIES — city / local income tax layers on top of state tax
// =============================================================================
// Only includes localities that actually reach rental income and/or capital gains.
// Wage/earnings-only municipal taxes (Ohio cities, Detroit and other MI cities,
// St. Louis / Kansas City earnings taxes, Kentucky occupational taxes, Colorado
// occupational privilege taxes) are intentionally excluded — they do not apply
// to net rental income or to capital gains from real estate.
//
// holdRate: locality rate applied to net rental income during the hold period
// saleRate: locality rate applied to gain at disposition
//   (Philadelphia SIT does not reach long-term gains from real estate, so its
//    saleRate is 0 while its holdRate is 3.74%.)
// =============================================================================
const LOCALITIES = {
  NY: [
    { id: 'NYC', name: 'New York City', holdRate: 0.03876, saleRate: 0.03876,
      note: 'NYC personal income tax, top bracket 3.876%; resident basis on worldwide income including rental income and capital gains.' },
    { id: 'YONKERS', name: 'Yonkers (resident)', holdRate: 0.01826, saleRate: 0.01826,
      note: 'Yonkers resident income tax surcharge equal to 16.75% of NY State tax. Effective ~1.83% at the top NY bracket.' }
  ],
  PA: [
    { id: 'PHILLY_SIT', name: 'Philadelphia (School Income Tax)', holdRate: 0.0374, saleRate: 0.0,
      note: 'Philadelphia School Income Tax at 3.74% on residents\u2019 net rental income (taxable unearned income). Long-term capital gains from real estate are generally not subject to SIT, so the locality layer drops out at disposition.' }
  ],
  MD: [
    { id: 'MD_320', name: 'Baltimore City / Howard / Montgomery / Prince George\u2019s (3.20%)', holdRate: 0.0320, saleRate: 0.0320,
      note: 'Top-tier MD county/city local income tax rate (3.20%), applied to residents on broad taxable income including rental income and capital gains.' },
    { id: 'MD_AA', name: 'Anne Arundel (2.81%)', holdRate: 0.0281, saleRate: 0.0281,
      note: 'Anne Arundel County local income tax (2.81%).' },
    { id: 'MD_AVG', name: 'Other MD County (\u22482.90%)', holdRate: 0.0290, saleRate: 0.0290,
      note: 'Representative rate for non-top-tier Maryland counties; verify against the specific county rate.' }
  ],
  IN: [
    { id: 'IN_MARION', name: 'Marion County / Indianapolis (2.02%)', holdRate: 0.0202, saleRate: 0.0202,
      note: 'Marion County local income tax, applied to resident AGI.' },
    { id: 'IN_AVG', name: 'Other Indiana County (\u22481.70%)', holdRate: 0.0170, saleRate: 0.0170,
      note: 'Indiana county LITs range roughly 0.5%\u20133.5%; ~1.70% is a representative middle.' }
  ],
  OR: [
    { id: 'PDX_TOP', name: 'Portland Metro + Multnomah PFA (top tier, 4.00%)', holdRate: 0.04, saleRate: 0.04,
      note: 'Metro Supportive Housing Services 1% + Multnomah County Preschool for All 3% on resident income above $250K single / $400K MFJ.' },
    { id: 'PDX_MID', name: 'Portland Metro + Multnomah PFA (mid tier, 2.50%)', holdRate: 0.025, saleRate: 0.025,
      note: 'Metro SHS 1% + Multnomah PFA 1.5% on resident income $125K\u2013$250K single / $200K\u2013$400K MFJ.' }
  ]
};

// Return the locality record for a given state + locality code, or null.
function getLocality(stateCode, localityCode) {
  if (!localityCode || localityCode === 'NONE') return null;
  const list = LOCALITIES[stateCode];
  if (!list) return null;
  return list.find(l => l.id === localityCode) || null;
}

// Return locality rate applied during hold (rental income, ordinary).
function getLocalityHoldRate(stateCode, localityCode) {
  const loc = getLocality(stateCode, localityCode);
  return loc ? loc.holdRate : 0;
}

// Return locality rate applied at disposition (gain on sale).
function getLocalitySaleRate(stateCode, localityCode) {
  const loc = getLocality(stateCode, localityCode);
  return loc ? loc.saleRate : 0;
}
...
// 27.5-year residential straight-line, half-year convention (legacy — retained for reference)
const SL_RESIDENTIAL_FIRST = 1 / 27.5 / 2;
const SL_RESIDENTIAL_FULL = 1 / 27.5;

// =============================================================================
// MID-MONTH CONVENTION (Reg. § 1.168(d)-2)
// Year-1 factor when placed in service in month M:  (12 - M + 0.5) / 12 / recovery
// Year-of-disposition factor when sold in month M:  (M - 0.5) / 12 / recovery
// All other years:  1 / recovery
// =============================================================================
function midMonthY1Factor(monthPlaced, isResidential) {
  const recovery = isResidential ? 27.5 : 39;
  return ((12 - monthPlaced + 0.5) / 12) / recovery;
}
function midMonthDispositionFactor(monthDisposed, isResidential) {
  const recovery = isResidential ? 27.5 : 39;
  return ((monthDisposed - 0.5) / 12) / recovery;
}
function annualSLFactor(isResidential) {
  return isResidential ? (1 / 27.5) : (1 / 39);
}
...
/* A restored value is whatever localStorage held — JSON.parse hands back an
   ...
   "Over N-year hold" sublabel in renderSummary (CodeQL js/xss-through-dom 30).
...
// =============================================================================
// LOCALITY DROPDOWN — populated dynamically based on selected state.
// Shows only when the active state has localities in the LOCALITIES table
// (currently NY, PA, MD, IN, OR). For all other states the row is hidden.
// =============================================================================
function populateLocalitiesForState() {
  const code = val('state');
  const list = LOCALITIES[code] || [];
  const localitySelect = document.getElementById('locality');
  const localityRow = document.getElementById('locality_row');
  if (!localitySelect || !localityRow) return;  // HTML doesn't have the dropdown yet
  if (list.length === 0) {
    localityRow.style.display = 'none';
    localitySelect.innerHTML = '<option value="NONE">None</option>';
    localitySelect.value = 'NONE';
  } else {
    localityRow.style.display = '';
    const opts = ['<option value="NONE">None &mdash; state only</option>']
      .concat(list.map(l => `<option value="${l.id}">${l.name}</option>`));
    localitySelect.innerHTML = opts.join('');
    localitySelect.value = 'NONE';
  }
  updateLocalityNote();
}

function updateLocalityNote() {
  const stateCode = val('state');
  const localitySelect = document.getElementById('locality');
  const noteEl = document.getElementById('locality_note');
  if (!localitySelect || !noteEl) return;
  const loc = getLocality(stateCode, localitySelect.value);
  if (!loc) {
    noteEl.innerHTML = 'Optional city/local income tax layer that stacks on top of the state rate. Choose one if the taxpayer is a resident of an enumerated locality.';
    return;
  }
  noteEl.innerHTML = `<strong>${loc.name}:</strong> ${loc.note}`;
}
...
// =============================================================================
function buildFederalDepreciation(buildingBasis, reclassPct, bonusRate, holdYears, monthPlaced, monthDisposed, isResidential) {
  ...
  const y1ShellFactor = midMonthY1Factor(monthPlaced, isResidential);
  const fullShellFactor = annualSLFactor(isResidential);
  const dispShellFactor = midMonthDispositionFactor(monthDisposed, isResidential);

  const yearly = [];
  for (let y = 1; y <= holdYears; y++) {
    let dep = 0;
    if (y === 1) {
      // Year 1: bonus + half-year MACRS Y1 on non-bonus reclass + mid-month shell
      dep += bonusDed;
      dep += reclassResidual * MACRS_5YR[0];
      dep += shellAmt * y1ShellFactor;
    } else if (y === holdYears) {
      // Year of disposition: full MACRS on reclass for the year + mid-month shell for partial year
      if (y - 1 < MACRS_5YR.length) {
        dep += reclassResidual * MACRS_5YR[y - 1];
      }
      dep += shellAmt * dispShellFactor;
    } else {
      // Intermediate years
      if (y - 1 < MACRS_5YR.length) {
        dep += reclassResidual * MACRS_5YR[y - 1];
      }
      dep += shellAmt * fullShellFactor;
    }
    yearly.push(dep);
  }
  return yearly;
...
function buildStateDepreciation(buildingBasis, reclassPct, holdYears, conforms, monthPlaced, monthDisposed, isResidential) {
...
function gatherInputsFromDOM() {
  ...
  const inputs = {
    ...
    dispositionStrategy: val('disposition_strategy') || 'sale',
    // Property type and month conventions (v2 — for proper mid-month convention)
    propertyType: val('property_type') || 'residential',
    monthPlaced: parseInt(val('month_placed_in_service')) || 1,
    monthDisposition: parseInt(val('month_disposition')) || 12
  };
...
function computeScenario(inputs) {
  ...
  // ----- Depreciation -----
  const isResidential = (inputs.propertyType !== 'commercial');
  const fedDep = buildFederalDepreciation(buildingBasis, effectiveReclassPct, inputs.bonusRate, inputs.holdPeriod, inputs.monthPlaced, inputs.monthDisposition, isResidential);
  const stateConforms = STATES[inputs.state].conforms_bonus;
  const stateDep = stateConforms === true ? fedDep : buildStateDepreciation(buildingBasis, effectiveReclassPct, inputs.holdPeriod, false, inputs.monthPlaced, inputs.monthDisposition, isResidential);
  ...
  // Locality (city/local) layer on top of state for hold and disposition
  const localityHoldRate = getLocalityHoldRate(inputs.state, inputs.locality);
  const localitySaleRate = getLocalitySaleRate(inputs.state, inputs.locality);
  const localityInfo = getLocality(inputs.state, inputs.locality);
  const combinedHoldRate = stateRate + localityHoldRate;
  ...
  for (let y = 1; y <= inputs.holdPeriod; y++) {
    ...
    if (stateHasTax) {
      ...
      // Locality (e.g., NYC, Yonkers) stacks on top of state on the same income basis.
      const stateOrdBaseline = (inputs.w2Wages + Math.max(inputs.businessIncome, 0) + inputs.otherNonBusiness) * stateRate;
      const localityOrdBaseline = (inputs.w2Wages + Math.max(inputs.businessIncome, 0) + inputs.otherNonBusiness) * localityHoldRate;
      ...
      const localityLTCGBaseline = inputs.ltCapGains * localityHoldRate;
      const stateBaseline = stateOrdBaseline + localityOrdBaseline + stateLTCGBaseline + localityLTCGBaseline;
      ...
      const localityOrdAfter = Math.max(0, inputs.w2Wages + inputs.businessIncome + inputs.otherNonBusiness + stateRentalUsable) * localityHoldRate;
      ...
      const localityLTCGAfter = inputs.ltCapGains * localityHoldRate;
      const stateAfter = stateOrdAfter + localityOrdAfter + stateLTCGAfter + localityLTCGAfter;
  ...
  if (stateHasTax) {
    ...
    // Locality layer on the gain (e.g., NYC, Yonkers, MD county). Philly SIT has
    // saleRate=0 because long-term real estate gains are not subject to SIT.
    localitySaleTax = Math.max(0, stateGain) * localitySaleRate;
  } else if (STATES[inputs.state].ltcg_treatment === 'wa_special') {
  ...
  // Combined state + locality tax at disposition (single line item flowing into
  // computeModeFullResults; the existing flow treats stateSaleTax as the
  // "state side" of the sale tax, and we extend it to include locality.)
  const stateAndLocalitySaleTax = stateSaleTax + localitySaleTax;

  // Mortgage payoff
  const mortgagePayoff = mortgage.yearlyData[saleYear - 1].endBalance;

  // Disposition strategy override:
  //   'sale'    — full federal + state tax (default; current logic)
  //   '1031'    — zero current tax (deferred into replacement property's basis)
  //   'stepup'  — zero current tax (permanent elimination under § 1014 basis step-up)
  const dispoStrategy = inputs.dispositionStrategy || 'sale';
  const dispoAppliesNoTax = (dispoStrategy === '1031' || dispoStrategy === 'stepup');

  // Compute totals + IRR for BOTH modes
  // useExitStrategy = the Donovan State Exit Strategy (residency change prior to
  // disposition). When true, the state-side and locality-side disposition tax is
  // zero because the seller is no longer a resident of the high-tax state/locality
  // and the property has no source nexus to it. Federal tax is unaffected.
  function computeModeFullResults(saleResult, useExitStrategy) {
    const exitApplied = !!useExitStrategy;
    // Apply disposition strategy: 1031 and step-up produce $0 current tax
    const effectiveSaleTaxFed = dispoAppliesNoTax ? 0 : saleResult.totalTax;
    // State portion of disposition tax: zero if 1031/stepup OR if state exit strategy applied
    const effectiveStateSaleTax = (dispoAppliesNoTax || exitApplied) ? 0 : stateSaleTax;
    const effectiveLocalitySaleTax = (dispoAppliesNoTax || exitApplied) ? 0 : localitySaleTax;
    const effectiveStateAndLocality = effectiveStateSaleTax + effectiveLocalitySaleTax;
    // Track deferred/eliminated tax (informational) — what would have been owed under a Sale
    const deferredTaxFed = dispoAppliesNoTax ? saleResult.totalTax : 0;
    const deferredTaxState = dispoAppliesNoTax ? stateSaleTax : 0;
    const deferredTaxLocality = dispoAppliesNoTax ? localitySaleTax : 0;
    // Track exit-strategy savings (state + locality) separate from deferral
    const exitStrategyStateSavings = exitApplied ? (stateSaleTax + localitySaleTax) : 0;

    const totalFed = effectiveSaleTaxFed;
    const eff = fedGain > 0 ? (totalFed / fedGain * 100) : 0;
    const netProceeds = netSalePrice - mortgagePayoff - effectiveSaleTaxFed - effectiveStateAndLocality;
    ...
    // Cumulative effective tax rate over the investment life — broken out:
    //   FEDERAL: sale-year federal tax minus cumulative federal operating savings
    //   STATE:   sale-year state + locality tax minus cumulative state + locality operating savings
    //   COMBINED: sum of both, divided by total economic gain
    // Total economic gain = cumulative NOI + capital appreciation
    const netFedTax = totalFed - cumFedTaxSavings;
    const netStateTax = effectiveStateAndLocality - cumStateTaxSavings;
    const netCombinedTax = netFedTax + netStateTax;
    ...
    function _rate(numerator) {
      if (totalEconomicGain <= 1000) return 0;
      let r = (numerator / totalEconomicGain) * 100;
      return Math.max(-100, Math.min(100, r));
    }
    const cumEffRate          = _rate(netFedTax);       // federal-only (legacy field; preserved)
    const cumStateEffRate     = _rate(netStateTax);     // state + locality only
    const cumCombinedEffRate  = _rate(netCombinedTax);  // federal + state + locality
    return {
      sale: saleResult,
      totalSaleTaxFed: totalFed,
      totalSaleTaxState: effectiveStateAndLocality,           // state + locality at disposition (after exit-strategy if applied)
      totalSaleTaxCombined: totalFed + effectiveStateAndLocality,
      effSaleRate: eff,
      cumEffRate: cumEffRate,                                 // federal-only (legacy)
      cumStateEffRate: cumStateEffRate,                       // state + locality
      cumCombinedEffRate: cumCombinedEffRate,                 // federal + state + locality
      netFedTax: netFedTax,
      netStateTax: netStateTax,                               // sale-year state - cumulative state savings
      netCombinedTax: netCombinedTax,
      netSaleProceeds: netProceeds,
      irr: computedIRR,
      totalProfit: totalProfit,
      roi: roi,
      totalReturn: totalProfit,
      // Disposition strategy detail
      dispositionStrategy: dispoStrategy,
      deferredTaxFed: deferredTaxFed,
      deferredTaxState: deferredTaxState,
      deferredTaxLocality: deferredTaxLocality,
      hypotheticalSaleTaxFed: saleResult.totalTax,  // what tax WOULD have been under a Sale
      // State exit strategy detail
      stateExitApplied: exitApplied,
      effectiveStateSaleTax: effectiveStateSaleTax,
      effectiveLocalitySaleTax: effectiveLocalitySaleTax,
      effectiveStateAndLocalitySaleTax: effectiveStateAndLocality,
      exitStrategyStateSavings: exitStrategyStateSavings
    };
  }

  // Pre-compute all four strategy-mode combinations:
  //   none  — neither strategy (standard)
  //   recap — federal recapture-allocation strategy only (existing "donovan")
  //   exit  — state exit strategy only (new; zeroes state + locality at disposition)
  //   both  — federal recap + state exit, layered
  const r_none  = computeModeFullResults(saleStandard, false);
  const r_recap = computeModeFullResults(saleDonovan,  false);
  const r_exit  = computeModeFullResults(saleStandard, true);
  const r_both  = computeModeFullResults(saleDonovan,  true);
  ...
  const year1StateTaxSavings = yearlyResults[0] ? (yearlyResults[0].stateTaxSavings || 0) : 0;
  ...
  return {
    // New canonical mode keys
    none: r_none,
    recap: r_recap,
    exit: r_exit,
    both: r_both,
    // Back-compat aliases (existing render code reads .standard and .donovan):
    //   .standard = no strategies (= none)
    //   .donovan  = recapture-allocation strategy on its own (= recap)
    standard: r_none,
    donovan: r_recap,
    // Headline savings figures
    strategySavings: r_none.totalSaleTaxFed - r_recap.totalSaleTaxFed,  // federal recap savings
    exitStrategySavings: r_exit.exitStrategyStateSavings,                // state + locality exit savings
    combinedSavings: (r_none.totalSaleTaxFed - r_both.totalSaleTaxFed) + r_both.exitStrategyStateSavings,
    common: {
      saleYear, finalValue, sellingCosts, netSalePrice,
      sellingCostsPct,
      fedAdjBasis, fedGain, dep1245, dep1250,
      mortgagePayoff,
      stateGain, stateSaleTax, localitySaleTax, stateAndLocalitySaleTax,
      stateAdjBasis, stateHasTax, stateConforms,
      stateName: STATES[inputs.state].name,
      locality: inputs.locality,
      localityInfo: localityInfo,
      localityHoldRate, localitySaleRate,
      cumFedTaxSavings, cumStateTaxSavings, nolBalance,
      cashFlowAfterTax: cumCashFlowAfterTax,
      cumNOI,
      year1NetCashBenefit,
      year1FedTaxSavings,
      year1StateTaxSavings,
      year1OpCashFlow,
      computedEffRate,
...
// =============================================================================
// STRATEGY MODE — TWO INDEPENDENT TOGGLES
// =============================================================================
// window.__strRecapStrategy : boolean — federal §1245 recapture-allocation strategy
// window.__strExitStrategy  : boolean — state exit (residency-change) strategy
// resolveStrategyMode()     : returns 'none' | 'recap' | 'exit' | 'both'
// resultsForCurrentMode()   : returns the pre-computed result for the active mode
// =============================================================================
function resolveStrategyMode() {
  const recap = !!window.__strRecapStrategy;
  const exit  = !!window.__strExitStrategy;
  if (recap && exit) return 'both';
  if (recap) return 'recap';
  if (exit)  return 'exit';
  return 'none';
}

function resultsForCurrentMode() {
  if (!window.__strResults) return null;
  return window.__strResults[resolveStrategyMode()];
}
...
function calculateSTR() {
  ...
  // Default: both strategy toggles off (= "standard / none" mode)
  window.__strRecapStrategy = false;
  window.__strExitStrategy  = false;
  // Back-compat: existing code reads window.__strMode === 'donovan'
  window.__strMode = 'standard';

  // Tier banner — surfaces which version the visitor is using and (in public)
  // a path to the full analysis. Renders inside #results so it's only visible
  // once an analysis has been run.
  const tierBannerEl = document.getElementById('tier_banner');
  if (tierBannerEl) {
    if (isPublic()) {
      tierBannerEl.innerHTML = `<div class="tier-banner tier-public">
        <div class="tier-banner-text"><strong>Public preview.</strong> You're viewing the open version of this analyzer. The \u00a7 469 eligibility analysis and Year 1 federal tax savings are shown openly; the multi-year projection, sale-year tax analysis, disposition strategy comparison, and Economic Profile scorecard are members-only.</div>
        <a href="engagement.html" class="tier-banner-cta">Learn about membership &rarr;</a>
      </div>`;
    } else {
      tierBannerEl.innerHTML = `<div class="tier-banner tier-client">
        <div class="tier-banner-text"><strong>Client analysis.</strong> Full version with multi-year projection, sale-year recapture allocation, disposition strategy comparison, and the Donovan Legal Tax Strategy and Exit Planning toggles enabled.</div>
      </div>`;
    }
  }
  ...
  // Render the strategy comparison panel (which has its own public/client gate)
  if (typeof renderStrategyComparison === 'function') renderStrategyComparison();
...
const HEAT_MAP_METRICS = {
  ...
  cumEffRate: {
    label: 'Cumulative Effective Federal Tax Rate',
    fmt: function(v) { return v.toFixed(1) + '%'; },
    extract: function(r) { return r.cumEffRate; },
    higher_is_better: false
  },
  cumStateEffRate: {
    label: 'Cumulative Effective State + Locality Rate',
    fmt: function(v) { return v.toFixed(1) + '%'; },
    extract: function(r) { return r.cumStateEffRate; },
    higher_is_better: false
  },
  cumCombinedEffRate: {
    label: 'Cumulative Effective Combined Rate (Fed + State)',
    fmt: function(v) { return v.toFixed(1) + '%'; },
    extract: function(r) { return r.cumCombinedEffRate; },
    higher_is_better: false
  }
};
...
function computeHeatMapCell(overrides) {
  ...
  try {
    ...
    return {
      ...
      cumStateEffRate: r.standard.cumStateEffRate,
      cumCombinedEffRate: r.standard.cumCombinedEffRate
    };
  } catch (e) {
    return null;
  }
...
function renderHeatMap() {
  // Public-tier gate: lock the heat map entirely (5x5 sensitivity grid is a
  // high-value firm asset; lock the whole grid).
  if (isPublic()) {
    let grid = '<div style="display:grid; grid-template-columns: repeat(5, 1fr); gap:3px; padding:1.5rem; background:#f5f5f0;">';
    for (let i = 0; i < 25; i++) grid += '<div style="height:48px; background:#dcdcd8; border-radius:2px;"></div>';
    grid += '</div>';
    document.getElementById('heat_map_grid').innerHTML = tierLock(grid, 'Sensitivity Heat Map \u2014 Member access', 'See how the after-tax IRR or any other metric responds to swings in purchase price, hold period, cost-segregation reclass percentage, mortgage rate, or down payment. Surfaces input fragility that a single point estimate hides.');
    return;
  }
  ...
  let html = '<div class="heat-map-table-wrapper" style="overflow-x:auto; -webkit-overflow-scrolling:touch;">';
...
function renderStateStatus(stateCode) {
  ...
  // Resolve active locality from the DOM (if any).
  const localityEl = document.getElementById('locality');
  const localityCode = localityEl ? localityEl.value : 'NONE';
  const loc = getLocality(stateCode, localityCode);
  ...
  // Locality row (only when state has tax and a locality is selected)
  const localityRows = (s.has_tax && loc) ? `
      <tr>
        <td style="padding: 0.25rem 0.5rem 0.25rem 0; color: #6b6b6b;">Locality (${loc.name})</td>
        <td style="padding: 0.25rem 0; font-weight: 600;">Hold ${(loc.holdRate * 100).toFixed(2)}% &middot; Sale ${(loc.saleRate * 100).toFixed(2)}%</td>
      </tr>
      <tr>
        <td style="padding: 0.25rem 0.5rem 0.25rem 0; color: #6b6b6b;">Combined State + Locality (top)</td>
        <td style="padding: 0.25rem 0; font-weight: 600; color: #1a1a1a;">${((s.rate + loc.holdRate) * 100).toFixed(2)}% on hold &middot; ${((s.rate + loc.saleRate) * 100).toFixed(2)}% on gain (ordinary basis)</td>
      </tr>
  ` : '';
  ...
  const conformityRows = s.has_tax ? `
    <table style="width: 100%; margin-top: 0.75rem; font-size: 0.82rem; border-collapse: collapse;">
      ...
      ${localityRows}
      <tr>
        <td style="padding: 0.25rem 0.5rem 0.25rem 0; color: #6b6b6b;">&sect; 168(k) Bonus Depreciation</td>
        <td style="padding: 0.25rem 0; font-weight: 600; color: ${s.conforms_bonus ? '#169B62' : '#d35400'};">${s.conforms_bonus ? 'Conforms' : 'Decoupled'}</td>
      </tr>
      <tr>
        <td style="padding: 0.25rem 0.5rem 0.25rem 0; color: #6b6b6b;">&sect; 461(l) Excess Business Loss</td>
        <td style="padding: 0.25rem 0; font-weight: 600; color: ${s.conforms_461l ? '#169B62' : '#d35400'};">${s.conforms_461l ? 'Conforms' : 'Decoupled'}</td>
      </tr>
      <tr>
        <td style="padding: 0.25rem 0.5rem 0.25rem 0; color: #6b6b6b;">&sect; 172 NOL 80% Rule</td>
        <td style="padding: 0.25rem 0; font-weight: 600; color: ${s.conforms_172_80 ? '#169B62' : '#d35400'};">${s.conforms_172_80 ? 'Conforms' : 'Decoupled'}</td>
      </tr>
    </table>
  ` : '';

  // Combined note: state + locality
  const localityNote = loc ? `<div style="font-size: 0.85rem; color: #6b6b6b; margin-top: 0.4rem;"><em>${loc.note}</em></div>` : '';
...
function renderSummary(data) {
  ...
  // Combined (fed + state + locality) is the headline number; fed and state are shown
  // alongside so the user can see what drives the combined.
  const combinedRate = (typeof data.cumCombinedEffRate === 'number') ? data.cumCombinedEffRate : data.cumEffRate;
  const stateRate    = (typeof data.cumStateEffRate    === 'number') ? data.cumStateEffRate    : 0;
  const fedRate      = data.cumEffRate;
  const cumRateColor = combinedRate < 0 ? '#169B62' : '#1a1a1a';
  const cumRateLabel = combinedRate < 0 ? 'Net tax-positive investment' : 'Federal + state tax over total economic gain';
  const fedStateBreak = `<div style="margin-top:0.35rem; font-size:0.78rem; color:#555; line-height:1.4;">Federal <strong>${fedRate.toFixed(2)}%</strong> &nbsp;\u00b7&nbsp; State + locality <strong>${stateRate.toFixed(2)}%</strong></div>`;
  ...
  // Year 1 federal vs state tax savings — surface both. year1NetCashBenefit already
  // includes both fed and state savings (cashFlowAfterTax composition); the sub-card
  // breaks it out so the state component is visible to the user.
  const y1Fed   = data.year1FedTaxSavings || 0;
  const y1State = data.year1StateTaxSavings || 0;
  const y1Op    = data.year1OpCashFlow || 0;
  const y1Break = `<div style="margin-top:0.35rem; font-size:0.78rem; color:#555; line-height:1.4;">Operating CF <strong>${fmt(y1Op)}</strong> &nbsp;\u00b7&nbsp; Fed savings <strong>${fmt(y1Fed)}</strong> &nbsp;\u00b7&nbsp; State savings <strong>${fmt(y1State)}</strong></div>`;

  // Net sale proceeds breakdown: gross - mortgage - federal tax - state+locality tax
  const npFed   = data.totalSaleTaxFed   || 0;
  const npState = data.totalSaleTaxState || 0;
  const npBreak = `<div style="margin-top:0.35rem; font-size:0.78rem; color:#555; line-height:1.4;">After federal sale tax <strong>${fmt(npFed)}</strong> &nbsp;\u00b7&nbsp; state + locality <strong>${fmt(npState)}</strong></div>`;

  // Cumulative operating tax savings split
  const cumFed   = data.cumFedTaxSavings   || 0;
  const cumState = data.cumStateTaxSavings || 0;
  const cumTotal = cumFed + cumState;

  // In public mode, only the headline "Year 1 Federal Tax Savings" card is shown
  // openly — the rest of the metrics (combined effective rate, cumulative tax savings
  // breakdown, after-tax IRR, net sale proceeds, total after-tax profit) are locked.
  // The Y1 federal-only card serves as the proof-of-concept teaser; clients see all.
  if (isPublic()) {
    const y1FedOnly = data.year1FedTaxSavings || 0;
    const yr1Op    = data.year1OpCashFlow || 0;
    const html = ratesNote + `
      <div class="summary-card">
        <div class="label">Year 1 Federal Tax Savings</div>
        <div class="value">${fmt(y1FedOnly)}</div>
        <div class="sublabel">Federal tax savings from depreciation and rental losses, Year 1 (public preview &mdash; state breakdown member-only)</div>
        <div style="margin-top:0.35rem; font-size:0.78rem; color:#555; line-height:1.4;">Operating CF <strong>${fmt(yr1Op)}</strong></div>
      </div>
      <div class="summary-card tier-locked-host" style="padding:0;">
        ${tierLock('<div style="padding:1rem 1.25rem; min-height:120px;"><div class="label">After-Tax IRR</div><div class="value">\u2588\u2588.\u2588\u2588%</div></div>', 'After-Tax IRR', 'Full IRR after federal + state tax across the hold period.')}
      </div>
      <div class="summary-card tier-locked-host" style="padding:0;">
        ${tierLock('<div style="padding:1rem 1.25rem; min-height:140px;"><div class="label">Cumulative Effective Tax Rate (Combined)</div><div class="value">\u2588.\u2588\u2588%</div><div class="sublabel">Federal \u2588.\u2588\u2588% \u00b7 State + locality \u2588.\u2588\u2588%</div></div>', 'Cumulative Effective Tax Rate', 'Combined federal + state + locality effective rate over the investment life. Negative when operating savings exceed sale-year tax.')}
      </div>
      <div class="summary-card tier-locked-host" style="padding:0;">
        ${tierLock('<div style="padding:1rem 1.25rem; min-height:140px;"><div class="label">Cumulative Tax Savings (Operating)</div><div class="value">$\u2588\u2588\u2588,\u2588\u2588\u2588</div><div class="sublabel">Federal $\u2588\u2588\u2588,\u2588\u2588\u2588 \u00b7 State + locality $\u2588\u2588,\u2588\u2588\u2588</div></div>', 'Cumulative Operating Savings', 'Federal + state tax savings across all hold years from depreciation and rental losses.')}
      </div>
      <div class="summary-card tier-locked-host" style="padding:0;">
        ${tierLock('<div style="padding:1rem 1.25rem; min-height:140px;"><div class="label">Net Sale Proceeds</div><div class="value">$\u2588\u2588\u2588,\u2588\u2588\u2588</div></div>', 'Net Sale Proceeds', 'After mortgage payoff, sale costs, federal tax, and state + locality tax.')}
      </div>
      <div class="summary-card tier-locked-host" style="padding:0;">
        ${tierLock('<div style="padding:1rem 1.25rem; min-height:140px;"><div class="label">Total After-Tax Profit</div><div class="value">$\u2588\u2588\u2588,\u2588\u2588\u2588</div></div>', 'Total After-Tax Profit', 'Cumulative cash flow + sale proceeds \u2212 initial investment, after federal + state tax.')}
      </div>
    `;
  ...
  const html = ratesNote + `
    <div class="summary-card">
      ...
      <div class="sublabel">${data.irr === null ? 'Cannot compute (degenerate cash flows)' : 'Over ' + (numYears(val('hold_period')) || 0) + '-year hold (after federal + state tax)'}</div>
    </div>
    <div class="summary-card">
      <div class="label">Cumulative Effective Tax Rate (Combined)</div>
      <div class="value" style="color: ${cumRateColor};">${combinedRate.toFixed(2)}%</div>
    ...
    <div class="summary-card">
      ...
      <div class="sublabel">Operating cash flow + federal + state tax savings, Year 1</div>
      ${y1Break}
    </div>
    <div class="summary-card">
      <div class="label">Cumulative Tax Savings (Operating)</div>
      <div class="value">${fmt(cumTotal)}</div>
      <div class="sublabel">From depreciation and rental losses, all hold years</div>
      <div style="margin-top:0.35rem; font-size:0.78rem; color:#555; line-height:1.4;">Federal <strong>${fmt(cumFed)}</strong> &nbsp;\u00b7&nbsp; State + locality <strong>${fmt(cumState)}</strong></div>
    </div>
    <div class="summary-card">
    ...
    <div class="summary-card">
      ...
      <div class="sublabel">Cumulative cash flow + sale proceeds &minus; initial investment (after federal + state tax)</div>
    </div>
  `;
  document.getElementById('summary_grid').innerHTML = html;
}
...
function renderProjectionTable(years, hasState) {
  // Public-tier gate: show the table structure as a locked teaser. The client
  // tier renders the full table below.
  if (isPublic()) {
    const placeholderRows = [
      ['Property Operations', ['NOI (3% growth)','Mortgage Interest','Federal Depreciation','Rental Net Income']],
      ['\u00a7 461(l) and NOL', ['Loss Usable Post-461(l)','NOL Generated','NOL Used','NOL Balance EOY']],
      ['Tax & Operating Cash Flow', ['Federal Tax Savings','State + Locality Tax Savings','Operating Cash Flow After Tax','Mortgage Balance EOY']],
      ['Acquisition & Disposition', ['Cash Outflow to Acquire (Yr 1)','Net Sale Proceeds at Disposition','Total Net Cash Flow (Acq + Ops + Disp)']]
    ];
    const yrCount = (years && years.length) || 7;
    const headerCells = '<th>Item</th>' + Array.from({length: yrCount}, (_, i) => `<th>Yr ${i+1}</th>`).join('') + '<th class="cum-col-header">Cumulative</th>';
    let bodyHTML = '';
    placeholderRows.forEach(([section, rows]) => {
      bodyHTML += `<tr class="section-header"><td colspan="${yrCount + 2}">${section}</td></tr>`;
      rows.forEach(label => {
        const cells = Array.from({length: yrCount}, () => '<td>\u2588\u2588\u2588\u2588\u2588</td>').join('');
        bodyHTML += `<tr><td>${label}</td>${cells}<td class="cum-col"><strong>\u2588\u2588\u2588\u2588\u2588</strong></td></tr>`;
      });
    });
    const placeholderTable = `<table class="multi-year-table"><thead><tr>${headerCells}</tr></thead><tbody>${bodyHTML}</tbody></table>`;
    document.getElementById('projection_table').outerHTML = `<div id="projection_table" class="tier-locked-host">${tierLock(placeholderTable, 'Multi-Year Projection \u2014 Member access', 'Year-by-year cash flow after federal + state tax, depreciation schedule, NOL carryforward tracking, and a Cumulative column summing the full hold period. Plus acquisition outflow and net sale proceeds rows that compose the full IRR series.')}</div>`;
    return;
  }
  // Pull the active mode's results so we can show acquisition outflow (Year 1)
  // and net sale proceeds at disposition (Sale Year), composed with the per-year
  // after-tax operating cash flow into a full "Total Net Cash Flow" row that
  // mirrors the IRR series. Adds a final "Cumulative" column.
  const _modeKey = (typeof resolveStrategyMode === 'function') ? resolveStrategyMode() : 'none';
  const _r = (window.__strResults && window.__strResults[_modeKey]) ? window.__strResults[_modeKey] : null;
  const _c = (window.__strResults && window.__strResults.common) ? window.__strResults.common : null;
  const initInv         = (_c && _c.initialInvestment) ? _c.initialInvestment : 0;
  const netSaleProceeds = (_r && typeof _r.netSaleProceeds === 'number') ? _r.netSaleProceeds : 0;
  const saleYear        = (_c && _c.saleYear) ? _c.saleYear : (years.length || 0);

  // Precompute per-year acquisition / disposition / total-net-cash-flow series.
  const acqByYear = years.map(y => (y.year === 1 ? -initInv : 0));
  const dispByYear = years.map(y => (y.year === saleYear ? netSaleProceeds : 0));
  const totalNetCFByYear = years.map((y, i) => acqByYear[i] + (y.cashFlowAfterTax || 0) + dispByYear[i]);

  const headerCells = ['<th>Item</th>']
    .concat(years.map(y => `<th>Yr ${y.year}</th>`))
    .concat(['<th class="cum-col-header">Cumulative</th>'])
    .join('');
  const headerRow = `<thead><tr>${headerCells}</tr></thead><tbody>`;

  // Generic row helper that auto-appends a cumulative column.
  // valueFn(y, i) returns the raw number; fmtFn formats it.
  function rowSum(label, valueFn) {
    let sum = 0;
    ...
    return `<tr><td>${label}</td>${cells}<td class="cum-col"><strong>${fmt(sum)}</strong></td></tr>`;
  }
  // Point-in-time (balance) rows: cumulative column shows end-of-hold balance, not a sum.
  function rowEnd(label, valueFn) {
    const vals = years.map(y => valueFn(y));
    const cells = vals.map(v => `<td>${fmt(v)}</td>`).join('');
    const endVal = vals[vals.length - 1] || 0;
    return `<tr><td>${label}</td>${cells}<td class="cum-col"><strong>${fmt(endVal)}</strong></td></tr>`;
  }
  function sectionHeader(label) {
    return `<tr class="section-header"><td colspan="${years.length + 2}">${label}</td></tr>`;
  }

  let body = '';
  body += sectionHeader('Property Operations');
  body += rowSum('NOI (3% growth)', y => y.noi);
  body += rowSum('Mortgage Interest', y => y.interest);
  body += rowSum('Federal Depreciation', y => y.fedDep);
  body += rowSum('Rental Net Income (Federal)', y => y.rentalIncomeFed);

  body += sectionHeader('\u00a7 461(l) and NOL');
  body += rowSum('Loss Usable (Post-461(l))', y => y.lossUsable461);
  body += rowSum('NOL Generated This Year', y => y.nolGenerated);
  body += rowSum('NOL Used (80% rule)', y => y.nolUsed);
  body += rowEnd('NOL Balance End of Year', y => y.nolBalance);

  body += sectionHeader('Tax & Operating Cash Flow');
  body += rowSum('Federal Tax Savings', y => y.fedTaxSavings);
  if (hasState) body += rowSum('State + Locality Tax Savings', y => y.stateTaxSavings);
  body += rowSum('Operating Cash Flow After Tax', y => y.cashFlowAfterTax);
  body += rowEnd('Mortgage Balance EOY', y => y.mortgageBalance);

  // Acquisition / disposition / total net cash flow rows — this is the IRR series
  // visible to the user, with the Year 1 acquisition outflow and the Sale-Year
  // disposition inflow surfaced explicitly.
  body += sectionHeader('Acquisition &amp; Disposition');
  body += rowSum('Cash Outflow to Acquire (Yr 1 only)', (y, i) => acqByYear[i]);
  body += rowSum('Net Sale Proceeds at Disposition', (y, i) => dispByYear[i]);
  body += rowSum('<strong>Total Net Cash Flow (Acq + Ops + Disp)</strong>', (y, i) => totalNetCFByYear[i]);
...
function renderSaleAnalysis(s) {
  // Public-tier gate: the sale-year tax analysis is the firm's signature panel —
  // §1245 / §1250 / LTCG / NIIT recapture allocation, state gain breakdown, the
  // DLTS and Exit Planning toggles, and the strategy savings callouts. All locked
  // in public mode with a structural teaser.
  if (isPublic()) {
    const placeholder = `
      <div style="background:#FFFFFF; padding:1.25rem 1.5rem; border-left:3px solid #169B62;">
        <div style="margin-bottom:0.75rem;"><strong>Sale at end of Year [\u2588]</strong></div>
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:0.5rem 1.5rem; font-size:0.9rem; line-height:1.6;">
          <div>Sale price (3% appreciation):</div><div style="text-align:right;">$\u2588\u2588\u2588,\u2588\u2588\u2588</div>
          <div>Less selling costs:</div><div style="text-align:right;">($\u2588\u2588,\u2588\u2588\u2588)</div>
          <div><strong>Net sale price:</strong></div><div style="text-align:right;"><strong>$\u2588,\u2588\u2588\u2588,\u2588\u2588\u2588</strong></div>
          <div>Federal adjusted basis:</div><div style="text-align:right;">$\u2588\u2588\u2588,\u2588\u2588\u2588</div>
          <div><strong>Federal gain:</strong></div><div style="text-align:right;"><strong>$\u2588\u2588\u2588,\u2588\u2588\u2588</strong></div>
        </div>
        <div style="margin-top:1rem;">
          ...
          <div style="display:grid; grid-template-columns:1fr 1fr; gap:0.5rem 1.5rem; font-size:0.88rem; line-height:1.6;">
            <div>\u00a7 1245 ordinary recapture:</div><div style="text-align:right;">$\u2588\u2588\u2588,\u2588\u2588\u2588</div>
            <div>\u00a7 1250 unrecaptured gain:</div><div style="text-align:right;">$\u2588\u2588\u2588,\u2588\u2588\u2588</div>
            <div>Long-term capital gain:</div><div style="text-align:right;">$\u2588\u2588\u2588,\u2588\u2588\u2588</div>
            <div>NIIT:</div><div style="text-align:right;">$\u2588,\u2588\u2588\u2588</div>
            <div><strong>Total federal tax on sale:</strong></div><div style="text-align:right;"><strong>$\u2588\u2588\u2588,\u2588\u2588\u2588</strong></div>
          </div>
        </div>
        <div style="margin-top:1rem; padding-top:1rem; border-top:1px dashed #d4d4d0;">
          <div style="font-family:'Gotham Bold',sans-serif; font-size:0.85rem; color:#169B62; letter-spacing:1px; text-transform:uppercase; margin-bottom:0.5rem;">State &amp; Locality Tax on Sale</div>
          <div style="display:grid; grid-template-columns:1fr 1fr; gap:0.5rem 1.5rem; font-size:0.88rem; line-height:1.6;">
            <div>State adjusted basis:</div><div style="text-align:right;">$\u2588\u2588\u2588,\u2588\u2588\u2588</div>
            <div>State gain:</div><div style="text-align:right;">$\u2588\u2588\u2588,\u2588\u2588\u2588</div>
            <div>State tax on sale:</div><div style="text-align:right;">$\u2588\u2588,\u2588\u2588\u2588</div>
            <div><strong>Total state + locality tax:</strong></div><div style="text-align:right;"><strong>$\u2588\u2588\u2588,\u2588\u2588\u2588</strong></div>
          </div>
        </div>
        <div style="margin-top:1rem; padding-top:1rem; border-top:2px solid #169B62; font-size:1.05rem; line-height:1.6;">
          <div style="display:flex; justify-content:space-between;"><strong>Net sale proceeds (after-tax, after payoff):</strong><strong style="color:#169B62;">$\u2588\u2588\u2588,\u2588\u2588\u2588</strong></div>
        </div>
      </div>`;
    document.getElementById('sale_analysis').innerHTML = tierLock(placeholder, 'Sale-Year Tax Analysis \u2014 Member access', 'Federal recapture allocation (\u00a7 1245 / \u00a7 1250 / LTCG / NIIT), state gain calculation (with state-specific decoupling from federal bonus depreciation), and the Donovan Legal Tax Strategy and Exit Planning toggles that quantify the firm-specific savings opportunities.');
    return;
  }
  // Resolve the active strategy state
  const isRecapMode = !!window.__strRecapStrategy;
  const isExitMode  = !!window.__strExitStrategy;
  const isStrategyMode = isRecapMode;  // back-compat: federal recap toggle
  const strategySavings = window.__strResults ? window.__strResults.strategySavings : 0;
  const exitStrategySavings = window.__strResults ? window.__strResults.exitStrategySavings : 0;

  // Determine disposition strategy from current results
  const dispoStrategy = window.__strResults && window.__strResults.standard
    ? (window.__strResults.standard.dispositionStrategy || 'sale')
    : 'sale';
  const isSale = dispoStrategy === 'sale';
  const is1031 = dispoStrategy === '1031';
  const isStepup = dispoStrategy === 'stepup';

  // Build disposition-strategy banner
  let dispoBanner = '';
  if (is1031) {
    const stdHypothetical = window.__strResults.standard.hypotheticalSaleTaxFed || 0;
    dispoBanner = `
      <div style="background: rgba(22,155,98,0.08); border-left: 4px solid #169B62; padding: 0.95rem 1.25rem; margin-bottom: 1rem; font-size: 0.9rem; line-height: 1.55;">
        <strong style="color: #107a4d; display: block; font-family: 'Gotham Bold', sans-serif; margin-bottom: 0.3rem;">&sect; 1031 LIKE-KIND EXCHANGE \u2014 DEFERRED DISPOSITION</strong>
        At the end of the hold period, the property is exchanged into like-kind replacement real estate under &sect; 1031. <strong>No federal or state tax is recognized at the exchange.</strong> The federal tax that would otherwise have been owed under a Sale (approximately <strong>${fmt(stdHypothetical)}</strong>) carries forward as a basis reduction in the replacement property and remains deferred until that property is sold in a taxable disposition, or eliminated entirely if held until death (&sect; 1014 step-up). The recapture and gain figures below reflect the hypothetical Sale outcome for reference only \u2014 actual current-year tax is zero.
      </div>
    `;
  } else if (isStepup) {
    const stdHypothetical = window.__strResults.standard.hypotheticalSaleTaxFed || 0;
    dispoBanner = `
      <div style="background: rgba(22,155,98,0.10); border-left: 4px solid #169B62; padding: 0.95rem 1.25rem; margin-bottom: 1rem; font-size: 0.9rem; line-height: 1.55;">
        <strong style="color: #107a4d; display: block; font-family: 'Gotham Bold', sans-serif; margin-bottom: 0.3rem;">ESTATE STEP-UP UNDER &sect; 1014 \u2014 PERMANENT ELIMINATION</strong>
        At the end of the hold period, the property is included in the decedent's gross estate and the heir's basis is stepped up to fair market value under &sect; 1014. Depreciation taken during the decedent's lifetime is <strong>not recaptured</strong>; the deferred gain is <strong>permanently eliminated</strong>. The federal tax that would otherwise have been owed under a Sale (approximately <strong>${fmt(stdHypothetical)}</strong>) is forgiven in its entirety. Operating losses claimed during life remain claimed; the basis step-up wipes out the inside basis history. This is the most tax-efficient disposition strategy. The recapture and gain figures below reflect the hypothetical Sale outcome for reference only \u2014 actual current-year and lifetime tax is zero.
      </div>
    `;
  }
  ...
  const html = `
    <div style="background: #FFFFFF; padding: 1.25rem 1.5rem; border-left: 3px solid #169B62;">
      ${dispoBanner}
      <div style="margin-bottom: 0.75rem;"><strong>${isSale ? 'Sale' : (is1031 ? 'Hypothetical sale (deferred under \u00a7 1031)' : 'Hypothetical sale (eliminated under \u00a7 1014)')} at end of Year ${s.saleYear}</strong></div>
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.5rem 1.5rem; font-size: 0.9rem; line-height: 1.6;">
      ...
      <div class="strategy-toggle-wrapper">
        ...
        <div class="strategy-toggle-label" style="margin-top: 0.85rem;">State Tax Treatment at Exit</div>
        <div class="strategy-toggle">
          <button type="button" class="strategy-toggle-btn ${!isExitMode ? 'active' : ''}" data-mode="standard" onclick="setExitStrategyMode('standard')">Resident at Sale</button>
          <button type="button" class="strategy-toggle-btn ${isExitMode ? 'active' : ''}" data-mode="donovan" onclick="setExitStrategyMode('donovan')">With Donovan Exit Planning</button>
        </div>
      </div>
        ...
        <div class="strategy-savings-callout show">
          ...
          <table class="savings-comparison-table">
            ...
            <tbody>
              ...
              ${std.totalSaleTaxState ? `
              <tr>
                <td>State + locality tax on sale</td>
                <td>${fmt(std.totalSaleTaxState)}</td>
                <td>${fmt(dvn.totalSaleTaxState)}</td>
                <td class="delta-improvement">${(std.totalSaleTaxState - dvn.totalSaleTaxState) > 0 ? '\u2212' + fmt(std.totalSaleTaxState - dvn.totalSaleTaxState).replace('-', '') : fmt(0)}</td>
              </tr>
              <tr>
                <td>Combined fed + state + locality tax on sale</td>
                <td>${fmt(std.totalSaleTaxCombined)}</td>
                <td>${fmt(dvn.totalSaleTaxCombined)}</td>
                <td class="delta-improvement">\u2212${fmt(std.totalSaleTaxCombined - dvn.totalSaleTaxCombined).replace('-', '')}</td>
              </tr>` : ''}
              <tr>
                <td>Cumulative effective federal tax rate</td>
              ...
              ${(typeof std.cumStateEffRate === 'number' && std.cumStateEffRate !== 0) ? `
              <tr>
                <td>Cumulative effective state + locality rate</td>
                <td>${std.cumStateEffRate.toFixed(2)}%</td>
                <td>${dvn.cumStateEffRate.toFixed(2)}%</td>
                <td class="delta-improvement">${(std.cumStateEffRate - dvn.cumStateEffRate) >= 0 ? '\u2212' + (std.cumStateEffRate - dvn.cumStateEffRate).toFixed(2) + '%' : '+' + (dvn.cumStateEffRate - std.cumStateEffRate).toFixed(2) + '%'}</td>
              </tr>
              <tr>
                <td><strong>Cumulative effective combined rate</strong></td>
                <td><strong>${std.cumCombinedEffRate.toFixed(2)}%</strong></td>
                <td><strong>${dvn.cumCombinedEffRate.toFixed(2)}%</strong></td>
                <td class="delta-improvement"><strong>\u2212${(std.cumCombinedEffRate - dvn.cumCombinedEffRate).toFixed(2)}%</strong></td>
              </tr>` : ''}
              <tr>
      ...
      ${isExitMode && exitStrategySavings > 0 && window.__strResults && s.stateHasTax ? (() => {
        // Build the parallel state-exit savings callout. Compare current mode vs the
        // counterfactual where the exit strategy was OFF (recap mode preserved).
        const r = window.__strResults;
        const counterKey = isRecapMode ? 'recap' : 'none';   // same recap mode, exit OFF
        const activeKey  = isRecapMode ? 'both'  : 'exit';   // same recap mode, exit ON
        const off = r[counterKey];
        const on  = r[activeKey];
        const stateDelta    = off.effectiveStateAndLocalitySaleTax - on.effectiveStateAndLocalitySaleTax;
        const proceedsDelta = on.netSaleProceeds - off.netSaleProceeds;
        const irrDelta      = (on.irr !== null && off.irr !== null) ? (on.irr - off.irr) * 100 : null;
        const profitDelta   = on.totalProfit - off.totalProfit;
        return `
        <div class="strategy-savings-callout show" style="margin-top: 1rem;">
          <div class="savings-header">\u2728 Benefits of Donovan Exit Planning</div>
          <div class="savings-headline-label">Total State${s.localityInfo ? ' + Locality' : ''} Tax Savings on Sale</div>
          <div class="savings-headline">${fmt(stateDelta)}</div>
          <table class="savings-comparison-table">
            <thead>
              <tr>
                <th>Metric</th>
                <th>Resident at Sale</th>
                <th>With Exit Planning</th>
                <th>Improvement</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td>State${s.localityInfo ? ' + locality' : ''} tax on sale</td>
                <td>${fmt(off.effectiveStateAndLocalitySaleTax)}</td>
                <td>${fmt(on.effectiveStateAndLocalitySaleTax)}</td>
                <td class="delta-improvement">\u2212${fmt(stateDelta).replace('-', '')}</td>
              </tr>
              <tr>
                <td>Net sale proceeds</td>
                <td>${fmt(off.netSaleProceeds)}</td>
                <td>${fmt(on.netSaleProceeds)}</td>
                <td class="delta-improvement">+${fmt(proceedsDelta).replace('-', '')}</td>
              </tr>
              <tr>
                <td>After-tax IRR</td>
                <td>${off.irr === null ? 'N/A' : (off.irr * 100).toFixed(2) + '%'}</td>
                <td>${on.irr === null ? 'N/A' : (on.irr * 100).toFixed(2) + '%'}</td>
                <td class="delta-improvement">${irrDelta === null ? 'N/A' : '+' + irrDelta.toFixed(2) + '%'}</td>
              </tr>
              <tr>
                <td>Total after-tax profit</td>
                <td>${fmt(off.totalProfit)}</td>
                <td>${fmt(on.totalProfit)}</td>
                <td class="delta-improvement">+${fmt(profitDelta).replace('-', '')}</td>
              </tr>
            </tbody>
          </table>
          <div class="savings-explainer">
            These results assume the disposition occurs after a bona fide change of domicile to a jurisdiction with no state or locality income tax on the gain, and that the property has no source nexus to the former state. The defensibility of the domicile change against a residency audit by the former state is fact-specific; clean execution of the strategy involves coordinated planning of the move and the sale. <strong>Engagement with qualified counsel is essential before disposition.</strong>
            <br><br>
            <a href="contact.html" class="savings-cta">Contact the Firm to Discuss</a>
          </div>
        </div>`;
      ...
      ${s.stateHasTax ? `
      <div style="margin-top: 1rem; padding-top: 1rem; border-top: 1px dashed #d4d4d0;">
        <div style="font-family: 'Gotham Bold', sans-serif; font-size: 0.85rem; color: #169B62; letter-spacing: 1px; text-transform: uppercase; margin-bottom: 0.5rem;">${s.stateName}${s.localityInfo ? ' &mdash; ' + s.localityInfo.name : ''} State${s.localityInfo ? ' &amp; Locality' : ''} Tax on Sale</div>
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 0.5rem 1.5rem; font-size: 0.88rem; line-height: 1.6;">
          ...
          <div>${s.stateName} state tax on gain:</div><div style="text-align: right;${s.stateExitApplied ? ' color: #6b6b6b; text-decoration: line-through;' : ''}">${fmt(s.stateSaleTax)}</div>
          ${s.localityInfo && s.localitySaleTax > 0 ? `
          <div>${s.localityInfo.name} locality tax on gain:</div><div style="text-align: right;${s.stateExitApplied ? ' color: #6b6b6b; text-decoration: line-through;' : ''}">${fmt(s.localitySaleTax)}</div>` : ''}
          ${s.localityInfo && s.localitySaleTax === 0 && s.localityInfo.saleRate === 0 ? `
          <div>${s.localityInfo.name} locality tax on gain:</div><div style="text-align: right; color: #6b6b6b;">$0 (gain not subject to ${s.localityInfo.name})</div>` : ''}
          <div><strong>Total state${s.localityInfo ? ' + locality' : ''} tax on sale:</strong></div><div style="text-align: right;"><strong>${s.stateExitApplied ? fmt(0) : fmt(s.stateAndLocalitySaleTax)}</strong></div>
          <div>Cumulative state${s.localityInfo ? ' + locality' : ''} tax savings (operating years):</div><div style="text-align: right; color: #169B62;">${fmt(s.cumStateTaxSavings || 0)}</div>
          <div><strong>Net state${s.localityInfo ? ' + locality' : ''} tax position over hold:</strong></div><div style="text-align: right;"><strong>${fmt((s.stateExitApplied ? 0 : s.stateAndLocalitySaleTax) - (s.cumStateTaxSavings || 0))}</strong></div>
        </div>
        ${s.stateExitApplied ? `
        <div style="margin-top: 0.85rem; padding: 0.75rem 1rem; background: rgba(22,155,98,0.08); border-left: 3px solid #169B62; font-size: 0.85rem; line-height: 1.55; color: #107a4d;">
          <strong>Donovan Exit Planning applied.</strong> The model assumes a bona fide change of domicile prior to disposition; ${s.stateName}${s.localityInfo ? ' and ' + s.localityInfo.name : ''} no longer has resident jurisdiction over the gain, and the property is in a no-tax state, eliminating state${s.localityInfo ? ' and locality' : ''} tax at exit. Aggregate state${s.localityInfo ? '/locality' : ''} savings on disposition: <strong>${fmt(s.exitStrategyStateSavings)}</strong>.
        </div>` : ''}
      </div>` : ''}
...
// =============================================================================
// DISPOSITION STRATEGY COMPARISON — Sale vs § 1031 vs § 1014 Estate Step-Up
// Computes all three dispositions for the current scenario and renders a
// side-by-side comparison panel. Surfaces the powerful planning result that
// estate step-up under § 1014 permanently eliminates the deferred tax,
// whereas § 1031 defers it (potentially indefinitely if subsequent dispositions
// also qualify, ultimately terminating in a § 1014 step-up at death).
// =============================================================================
function renderDispositionComparison() {
  if (!window.__strResults) return;
  // Public-tier gate: the Sale vs § 1031 vs Estate Step-Up comparison is the
  // single most distinctive panel in the analyzer. Lock with a 3-column teaser
  // that previews the row structure but blurs values.
  if (isPublic()) {
    const placeholder = `
      <div class="disposition-comparison">
        <div class="dispo-panel-header">
          <div class="dispo-panel-title">Disposition Strategy Comparison</div>
          <div class="dispo-panel-subtitle">Same operating scenario, three different end-of-hold tax outcomes.</div>
        </div>
        <div class="dispo-table-wrapper">
        <table class="dispo-table">
          <thead>
            <tr>
              <th class="dispo-row-label"></th>
              <th class="dispo-col"><div class="dispo-col-name">Sale</div><div class="dispo-col-cite">Full recognition</div></th>
              <th class="dispo-col"><div class="dispo-col-name">\u00a7 1031 Exchange</div><div class="dispo-col-cite">Deferred</div></th>
              <th class="dispo-col"><div class="dispo-col-name">Estate Step-Up</div><div class="dispo-col-cite">\u00a7 1014 \u2014 permanent</div></th>
            </tr>
          </thead>
          <tbody>
            ${['Federal tax at disposition','State + locality tax at disposition','Combined tax at disposition','Net cash proceeds','Deferred tax remaining / Permanent savings','Total profit over hold','After-tax IRR','Cumulative effective federal tax rate','Cumulative effective state + locality rate','Cumulative effective combined rate'].map(label => `<tr><td class="dispo-row-label">${label}</td><td class="dispo-cell">\u2588\u2588\u2588,\u2588\u2588\u2588</td><td class="dispo-cell">\u2588\u2588\u2588,\u2588\u2588\u2588</td><td class="dispo-cell">\u2588\u2588\u2588,\u2588\u2588\u2588</td></tr>`).join('')}
          </tbody>
        </table>
        </div>
      </div>`;
    document.getElementById('disposition_comparison').innerHTML = tierLock(placeholder, 'Disposition Strategy Comparison \u2014 Member access', 'Side-by-side outcomes for taxable Sale, \u00a7 1031 Like-Kind Exchange, and \u00a7 1014 Estate Step-Up. Shows the dollar value of the deferred liability under \u00a7 1031 vs the permanent elimination under \u00a7 1014, and quantifies the cumulative effective tax rate (federal, state + locality, and combined) under each path.');
    return;
  }
  const r = window.__strResults;
  const c = r.common;
  const modeKey = resolveStrategyMode();   // 'none' | 'recap' | 'exit' | 'both'
  const isStrategyMode = (modeKey === 'recap' || modeKey === 'both');  // back-compat semantic
  const base = r[modeKey];

  // The standard sale tax (what would have been owed under a Sale, fed + state + locality)
  // hypotheticalSaleTaxFed is the federal tax under the active recapture-strategy mode;
  // for state, we reflect the active exit-strategy mode (zero if exit applied).
  const hypotheticalSaleTaxFed = base.hypotheticalSaleTaxFed || base.totalSaleTaxFed || 0;
  const rawStateAndLocality = c.stateAndLocalitySaleTax || c.stateSaleTax || 0;
  const hypotheticalStateSaleTax = base.stateExitApplied ? 0 : rawStateAndLocality;

  // For Sale: numbers as currently computed under Sale strategy
  // We need to compute Net Proceeds, Total Profit, and IRR under each disposition
  const initInv = c.initialInvestment || 0;
  const cumCashFlow = c.cashFlowAfterTax || 0;

  // ============================================================================
  // ECONOMIC FRAMING — three exit events evaluated AT the disposition date.
  // The model does not project beyond the disposition date.
  // ============================================================================
  //   SALE: Tax recognized. Investor receives net cash. No further tax obligation.
  //   § 1031: Tax DEFERRED. Gross proceeds roll into replacement property's basis.
  //     A deferred tax liability of $hypotheticalSaleTax[Fed+State] follows the
  //     property — quantified in the dedicated row below — but is not realized at
  //     this event. Whether it ever comes due depends on subsequent events that
  //     this comparison does not project.
  //   STEP-UP (§ 1014): Tax PERMANENTLY ELIMINATED at this event (owner's death).
  //     Heirs receive property at FMV basis; deferred recapture forgiven.
  //
  // §1031 and Step-Up are mathematically equivalent at the disposition date for
  // Net Cash, Total Profit, and IRR (both: $0 tax paid, full gross proceeds).
  // The qualitative and dollar difference — $300K of deferred liability still
  // attached to §1031 vs $0 attached to Step-Up — is quantified in the dedicated
  // "Deferred tax remaining / Permanent savings" row, which IS the answer to
  // "what is the dollar benefit of the step-up to heirs."
  // ============================================================================

  // SALE: full federal + state tax recognized at this disposition
  const saleNetProceeds = c.netSalePrice - c.mortgagePayoff - hypotheticalSaleTaxFed - hypotheticalStateSaleTax;
  const saleTotalProfit = cumCashFlow + saleNetProceeds - initInv;

  // § 1031: tax deferred at this event — gross proceeds roll into replacement basis
  const exch1031NetProceeds = c.netSalePrice - c.mortgagePayoff;
  const exch1031TotalProfit = cumCashFlow + exch1031NetProceeds - initInv;

  // STEP-UP: tax eliminated at this event — gross proceeds to heirs at FMV basis
  const stepupNetProceeds = exch1031NetProceeds;  // same gross dollar outcome at disposition
  const stepupTotalProfit = exch1031TotalProfit;

  // Dollar quantification of the deferred liability (§ 1031) vs permanent elimination (Step-Up)
  const stepupTaxSavings = hypotheticalSaleTaxFed + hypotheticalStateSaleTax;

  // IRR for each: compute via the existing logic
  function computeIRR(netProceedsAtSale) {
    const yrs = c.yearlyResults;
    const cf = [-initInv];
    for (let y = 1; y < c.saleYear; y++) cf.push(yrs[y - 1].cashFlowAfterTax);
    cf.push(yrs[c.saleYear - 1].cashFlowAfterTax + netProceedsAtSale);
    const allNeg = cf.every(x => x <= 0);
    const allPos = cf.every(x => x >= 0);
    if (allNeg || allPos || cf[0] >= 0) return null;
    let lo = -0.99, hi = 10.0;
    for (let i = 0; i < 300; i++) {
      const mid = (lo + hi) / 2;
      const v = cf.reduce((s, c, t) => s + c / Math.pow(1 + mid, t), 0);
  ...
  const saleIRR = computeIRR(saleNetProceeds);
  const exch1031IRR = computeIRR(exch1031NetProceeds);
  const stepupIRR = computeIRR(stepupNetProceeds);

  // Cumulative effective federal tax rate over the investment life for each strategy,
  // evaluated AT the disposition date with NO projection beyond that point.
  //
  //   SALE: paid the tax at disposition. Net fed tax = saleTax - cumOperatingSavings.
  //
  //   § 1031: $0 tax paid at disposition (deferred). Net fed tax through this date
  //     = -cumOperatingSavings (operating savings only). The deferred liability
  //     follows the property into the replacement basis but is NOT realized at
  //     this event, so it is excluded from this rate. The qualitative encumbrance
  //     is captured separately in the "Deferred tax remaining" row.
  //
  //   STEP-UP (§ 1014): $0 tax paid at disposition (eliminated). Net fed tax
  //     through this date = -cumOperatingSavings. Identical numerical outcome
  //     to § 1031 at this event; the difference is qualitative (permanent
  //     elimination vs deferred), shown in the dedicated rows.
  // ---------------------------------------------------------------------------------
  // BUG FIX (Batch C.7): Previous code read base.sale.fedGain which doesn't exist on
  // the saleResult object — only on the common object. NaN propagated to "N/A".
  // FRAMING (Batch C.10): Comparison evaluates the three exits at the disposition
  // date without projecting beyond that point. § 1031 and Step-Up are mathematically
  // equivalent at this event; the dollar/qualitative difference is captured in the
  // dedicated "Deferred tax remaining / Permanent savings" row.
  // Cumulative effective tax rate over the investment life — broken out fed / state / combined.
  //   SALE: tax paid at disposition (federal + state + locality)
  //   § 1031 and STEP-UP: $0 federal AND $0 state paid at this event; only operating
  //     savings recognized. Numerically the SAME at the disposition date — the
  //     qualitative difference (deferred liability vs permanent elimination) is
  //     captured in the dedicated "Deferred tax remaining / Permanent savings" row.
  //   The COMBINED rate is the headline that the user asked be sensitive to BOTH
  //   federal recap strategy AND state exit strategy AND disposition strategy.
  const cumFedTaxSavings = c.cumFedTaxSavings || 0;
  const cumStateTaxSavings = c.cumStateTaxSavings || 0;
  const cumNOI = c.cumNOI || 0;
  const fedGain = c.fedGain || 0;
  const cumDepreciation = (c.dep1245 || 0) + (c.dep1250 || 0);
  const priceAppreciation = fedGain - cumDepreciation;
  const totalEconomicGain = cumNOI + priceAppreciation;

  function effRate(netTax) {
    if (!isFinite(totalEconomicGain) || totalEconomicGain <= 1000) return null;
    const r = (netTax / totalEconomicGain) * 100;
    if (!isFinite(r)) return null;
    return Math.max(-200, Math.min(200, r));
  }
  // SALE: tax paid at disposition
  const saleNetFedTax       = hypotheticalSaleTaxFed   - cumFedTaxSavings;
  const saleNetStateTax     = hypotheticalStateSaleTax - cumStateTaxSavings;
  const saleNetCombinedTax  = saleNetFedTax + saleNetStateTax;
  // § 1031 and STEP-UP: $0 fed + $0 state at this event; only operating savings recognized
  const deferNetFedTax      = 0 - cumFedTaxSavings;
  const deferNetStateTax    = 0 - cumStateTaxSavings;
  const deferNetCombinedTax = deferNetFedTax + deferNetStateTax;
  const saleCumEffRate          = effRate(saleNetFedTax);
  const saleCumStateEffRate     = effRate(saleNetStateTax);
  const saleCumCombinedEffRate  = effRate(saleNetCombinedTax);
  const exch1031CumEffRate         = effRate(deferNetFedTax);
  const exch1031CumStateEffRate    = effRate(deferNetStateTax);
  const exch1031CumCombinedEffRate = effRate(deferNetCombinedTax);
  const stepupCumEffRate           = effRate(deferNetFedTax);
  const stepupCumStateEffRate      = effRate(deferNetStateTax);
  const stepupCumCombinedEffRate   = effRate(deferNetCombinedTax);

  // Determine which column the user actually selected
  const selected = base.dispositionStrategy || 'sale';

  // Build the comparison table
  function pct(v) { return (v == null || isNaN(v)) ? 'N/A' : (v * 100).toFixed(2) + '%'; }
  function pctR(v) { return (v == null || isNaN(v)) ? 'N/A' : v.toFixed(2) + '%'; }

  const html = `
    <div class="disposition-comparison">
      <div class="dispo-panel-header">
        <div class="dispo-panel-title">Disposition Strategy Comparison</div>
        <div class="dispo-panel-subtitle">Same operating scenario, three different end-of-hold tax outcomes. Selected strategy is highlighted; the others are shown for comparison.</div>
      </div>
      <div class="dispo-table-wrapper">
      <table class="dispo-table">
        <thead>
          <tr>
            <th class="dispo-row-label"></th>
            <th class="dispo-col${selected === 'sale' ? ' dispo-col-selected' : ''}">
              <div class="dispo-col-name">Sale</div>
              <div class="dispo-col-cite">Full recognition</div>
            </th>
            <th class="dispo-col${selected === '1031' ? ' dispo-col-selected' : ''}">
              <div class="dispo-col-name">&sect; 1031 Exchange</div>
              <div class="dispo-col-cite">Deferred</div>
            </th>
            <th class="dispo-col${selected === 'stepup' ? ' dispo-col-selected' : ''}">
              <div class="dispo-col-name">Estate Step-Up</div>
              <div class="dispo-col-cite">&sect; 1014 \u2014 permanent</div>
            </th>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td class="dispo-row-label">Federal tax at disposition</td>
            <td class="dispo-cell ${hypotheticalSaleTaxFed > 0 ? 'dispo-bad' : ''}">${fmt(hypotheticalSaleTaxFed)}</td>
            <td class="dispo-cell dispo-good">$0 <span class="dispo-cell-note">(deferred)</span></td>
            <td class="dispo-cell dispo-best">$0 <span class="dispo-cell-note">(eliminated)</span></td>
          </tr>
          <tr>
            <td class="dispo-row-label">State + locality tax at disposition</td>
            <td class="dispo-cell ${hypotheticalStateSaleTax > 0 ? 'dispo-bad' : ''}">${fmt(hypotheticalStateSaleTax)}${hypotheticalStateSaleTax === 0 && base.stateExitApplied ? ' <span class="dispo-cell-note">(Exit Planning applied)</span>' : ''}</td>
            <td class="dispo-cell dispo-good">$0 <span class="dispo-cell-note">(deferred / state varies)</span></td>
            <td class="dispo-cell dispo-best">$0 <span class="dispo-cell-note">(eliminated)</span></td>
          </tr>
          <tr>
            <td class="dispo-row-label"><strong>Combined tax at disposition</strong></td>
            <td class="dispo-cell ${(hypotheticalSaleTaxFed + hypotheticalStateSaleTax) > 0 ? 'dispo-bad' : 'dispo-good'}"><strong>${fmt(hypotheticalSaleTaxFed + hypotheticalStateSaleTax)}</strong></td>
            <td class="dispo-cell dispo-good"><strong>$0</strong></td>
            <td class="dispo-cell dispo-best"><strong>$0</strong></td>
          </tr>
          <tr>
            <td class="dispo-row-label">Net cash proceeds</td>
            <td class="dispo-cell">${fmt(saleNetProceeds)}</td>
            <td class="dispo-cell">${fmt(exch1031NetProceeds)} <span class="dispo-cell-note">into replacement property</span></td>
            <td class="dispo-cell">${fmt(stepupNetProceeds)} <span class="dispo-cell-note">to heirs at stepped-up basis</span></td>
          </tr>
          <tr>
            <td class="dispo-row-label">Deferred tax remaining / Permanent savings</td>
            <td class="dispo-cell">$0 <span class="dispo-cell-note">(paid at sale)</span></td>
            <td class="dispo-cell dispo-bad">${fmt(stepupTaxSavings)} <span class="dispo-cell-note">fed ${fmt(hypotheticalSaleTaxFed)} + state ${fmt(hypotheticalStateSaleTax)} \u2014 deferred liability attached to replacement basis</span></td>
            <td class="dispo-cell dispo-best">${fmt(stepupTaxSavings)} <span class="dispo-cell-note">fed ${fmt(hypotheticalSaleTaxFed)} + state ${fmt(hypotheticalStateSaleTax)} \u2014 permanently eliminated under &sect;&nbsp;1014</span></td>
          </tr>
          <tr>
            <td class="dispo-row-label">Total profit over hold</td>
            <td class="dispo-cell">${fmt(saleTotalProfit)}</td>
            <td class="dispo-cell">${fmt(exch1031TotalProfit)}</td>
            <td class="dispo-cell">${fmt(stepupTotalProfit)}</td>
          </tr>
          <tr>
            <td class="dispo-row-label">After-tax IRR</td>
            <td class="dispo-cell">${pct(saleIRR)}</td>
            <td class="dispo-cell">${pct(exch1031IRR)}</td>
            <td class="dispo-cell">${pct(stepupIRR)}</td>
          </tr>
          <tr>
            <td class="dispo-row-label">Cumulative effective federal tax rate</td>
            <td class="dispo-cell">${pctR(saleCumEffRate)}</td>
            <td class="dispo-cell">${pctR(exch1031CumEffRate)}${exch1031CumEffRate !== null ? '<div class="dispo-cell-note">(deferred at this event)</div>' : ''}</td>
            <td class="dispo-cell">${pctR(stepupCumEffRate)}${stepupCumEffRate !== null ? '<div class="dispo-cell-note">(eliminated under \u00a7&nbsp;1014)</div>' : ''}</td>
          </tr>
          <tr>
            <td class="dispo-row-label">Cumulative effective state + locality rate</td>
            <td class="dispo-cell">${pctR(saleCumStateEffRate)}</td>
            <td class="dispo-cell">${pctR(exch1031CumStateEffRate)}${exch1031CumStateEffRate !== null ? '<div class="dispo-cell-note">(deferred / state varies)</div>' : ''}</td>
            <td class="dispo-cell">${pctR(stepupCumStateEffRate)}${stepupCumStateEffRate !== null ? '<div class="dispo-cell-note">(eliminated under \u00a7&nbsp;1014)</div>' : ''}</td>
          </tr>
          <tr>
            <td class="dispo-row-label"><strong>Cumulative effective combined rate</strong></td>
            <td class="dispo-cell"><strong>${pctR(saleCumCombinedEffRate)}</strong></td>
            <td class="dispo-cell"><strong>${pctR(exch1031CumCombinedEffRate)}</strong>${exch1031CumCombinedEffRate !== null ? '<div class="dispo-cell-note">(deferred at this event)</div>' : ''}</td>
            <td class="dispo-cell"><strong>${pctR(stepupCumCombinedEffRate)}</strong>${stepupCumCombinedEffRate !== null ? '<div class="dispo-cell-note">(eliminated under \u00a7&nbsp;1014)</div>' : ''}</td>
          </tr>
          <tr>
            <td class="dispo-row-label">Tax treatment of deferred gain</td>
            <td class="dispo-cell dispo-cell-text">Recognized at sale; tax paid now.</td>
            <td class="dispo-cell dispo-cell-text">Carries into replacement basis. Comes due when replacement is sold in a taxable disposition (or eliminated if held until death).</td>
            <td class="dispo-cell dispo-cell-text"><strong>Permanently eliminated.</strong> Heir's basis = FMV at death; depreciation not recaptured; deferred gain forgiven.</td>
          </tr>
        </tbody>
      </table>
      </div>
      <div class="dispo-footnote">
        <strong>Framing.</strong> This comparison evaluates three exit events &mdash; Sale, &sect; 1031 Exchange, and Estate Step-Up &mdash; occurring on the disposition date selected for the analysis. Cash, profit, and IRR figures reflect outcomes AT that date and do not project subsequent events. At the disposition date, &sect; 1031 and Step-Up are mathematically equivalent (both: $0 tax paid, full gross proceeds available), but the dollar economic difference is real and significant: under &sect; 1031 a deferred tax liability of the amount shown remains attached to the replacement property&rsquo;s basis, while under Step-Up the same dollar amount is permanently extinguished under &sect; 1014. That dollar difference &mdash; the &ldquo;Deferred tax remaining / Permanent savings&rdquo; row &mdash; is the answer to &ldquo;what is the dollar benefit of the Step-Up to the heirs.&rdquo;
        <br/><br/>
        <strong>Strategy interactions.</strong> Two interactions are worth noting because they affect which cells move when you toggle the strategies above the Sale Year Tax Analysis. <em>Donovan Legal Tax Strategy</em> (federal recapture-allocation) affects only the <strong>Sale</strong> column; under &sect; 1031 and Step-Up no federal sale tax is paid in the first place, so there is no recapture to re-allocate, and the dollar benefit of DLTS is captured implicitly inside the Step-Up column&rsquo;s &ldquo;Permanent savings&rdquo; figure (which equals the hypothetical sale tax that DLTS would have produced). <em>Donovan Exit Planning</em> (state residency change) zeroes the state and locality sale tax under any disposition strategy; once Exit Planning is applied, all three columns show $0 of state sale tax, and the cumulative state rate is identical across the three columns &mdash; the only differentiator between them at that point is the federal column and the deferred-tax row.
        <br/><br/>
        <strong>Note on planning implications.</strong> The &sect; 1031 column assumes the exchange satisfies the like-kind, 45-day identification, 180-day acquisition, and qualified-intermediary requirements, with no boot received and full debt replacement &mdash; actual exchanges with boot or debt relief produce partial gain recognition. State conformity to &sect; 1031 varies (most states conform; PA historically did not but now conforms for individuals as of 2023; verify state treatment before exchange). The Estate Step-Up column assumes the property is held in the decedent&rsquo;s individual or revocable trust name (or otherwise included in the gross estate) and that the &sect; 1014 step-up survives any future tax law changes; community property in qualifying states receives a full step-up on both halves under &sect; 1014(b)(6). Operating losses claimed during life remain claimed under both deferral paths and are not recaptured at death.
      </div>
    </div>
  `;

  const target = document.getElementById('disposition_comparison');
...
function computeVerdict(mode) {
  ...
  // 5. Cumulative Effective Tax Rate — use COMBINED (fed + state + locality) so the
  // scorecard reflects the same headline number shown in the Summary panel and the
  // Disposition Strategy Comparison. The federal-only rate is preserved on r.cumEffRate
  // for legacy callers and the CSV export.
  const combinedRate = (typeof r.cumCombinedEffRate === 'number') ? r.cumCombinedEffRate : r.cumEffRate;
  let v, n;
  if (combinedRate < 0) { v = 'favorable'; n = 'Net tax-positive over investment life (combined fed + state)'; }
  else if (combinedRate < 10) { v = 'favorable'; n = 'Low effective tax burden (combined fed + state)'; }
  else if (combinedRate < 20) { v = 'solid'; n = 'Moderate effective tax burden (combined fed + state)'; }
  else if (combinedRate < 30) { v = 'marginal'; n = 'Significant tax burden (combined fed + state)'; }
  else { v = 'unfavorable'; n = 'High effective tax burden (combined fed + state)'; }
  scorecard.push({ label: 'Cumulative Effective Tax Rate', value: combinedRate.toFixed(2) + '%', verdict: v, note: n });
  ...
  // Sale tax / cumulative operating tax savings. Use COMBINED (fed + state + locality)
  // for both numerator and denominator so the ratio is meaningful when state taxes
  // are material. The metric surfaces the risk that recapture + gain dwarfs the
  // operating shield. BUT — at high cap rates, ops produce enough cash that the tax
  // shield is small or zero. In that case, the ratio is meaningless (small denominator
  // inflates the ratio). Suppress the unfavorable scoring when operating economics
  // carry the investment on their own. Uses the ACTIVE mode's sale tax (which reflects
  // both DLTS and Exit Planning) rather than the hard-coded Standard.
  const dispoTaxCombined = (r.totalSaleTaxFed || 0) + (r.totalSaleTaxState || 0);
  const opSavingsCombined = (c.cumFedTaxSavings || 0) + (c.cumStateTaxSavings || 0);
  const opCfStrong = cumOpCFPreTax > initInv * 0.5;  // ops alone return >50% of capital
  const dispoRatio = (opSavingsCombined > 0) ? dispoTaxCombined / opSavingsCombined : 999;
  let dv, dn;
  if (opCfStrong && opSavingsCombined < (initInv * 0.3)) {
  ...
  } else if (dispoTaxCombined === 0) {
    // Disposition strategy zeros the tax entirely (§ 1031 or Step-Up, possibly combined with Exit Planning)
    dv = 'favorable';
    dn = 'Disposition strategy eliminates sale tax — no tax friction at exit';
  ...
  scorecard.push({ label: 'Disposition Tax Risk', value: (opCfStrong && opSavingsCombined < (initInv * 0.3)) ? 'N/A (ops-funded)' : (dispoTaxCombined === 0 ? 'N/A ($0 sale tax)' : (dispoRatio < 100 ? dispoRatio.toFixed(2) + 'x' : 'High')), verdict: dv, note: dn });
...
function renderFinancingHealth() {
  ...
  const html = `
    <div style="background: #FFFFFF; padding: 1.5rem; margin: 0 0 1.75rem 0; border: 1px solid #d4d4d0;">
      ...
      <div class="financing-health-grid">
        ...
        <div style="border: 2px solid ${dscrColor}; padding: 1.25rem; background: ${dscrColor}15;">
          <div class="fh-card-label" style="font-family: 'Open Sans', sans-serif; font-size: 0.72rem; letter-spacing: 1.5px; text-transform: uppercase; color: ${dscrColor}; font-weight: 700; margin-bottom: 0.4rem;"><span class="fh-label-full">Debt Service Coverage Ratio (Year 1)</span><span class="fh-label-short">DSCR (Year 1)</span></div>
...
function renderVerdict(mode) {
  ...
  let scorecardHtml = '';
  if (v.scorecard) {
    // Public-tier gate: lock the full scorecard but show the row structure so
    // visitors see what's being analyzed. The eligibility verdict heading and CTA
    // remain visible in both tiers.
    if (isPublic()) {
      let placeholder = '<div class="scorecard-panel"><div class="scorecard-panel-label">Scorecard</div>';
      ['After-tax IRR','Operating Cash Flow (Pre-Tax-Shield)','Year 1 Cash Benefit (% of capital invested)','\u00a7 469 Eligibility','Cumulative Effective Tax Rate','Total After-Tax Profit','NOI Growth Trajectory','Disposition Tax Risk'].forEach(label => {
        placeholder += '<div class="scorecard-row">' +
          '<div class="scorecard-icon"><span style="color:#6b6b6b;">\u2014</span></div>' +
          '<div class="scorecard-label">' + label + '</div>' +
          '<div class="scorecard-note">\u2588\u2588\u2588\u2588\u2588\u2588\u2588\u2588\u2588\u2588\u2588\u2588\u2588</div>' +
          '<div class="scorecard-value">\u2588.\u2588\u2588</div>' +
        '</div>';
      });
      placeholder += '</div>';
      scorecardHtml = tierLock(placeholder, 'Economic Profile Scorecard \u2014 Member access', 'Eight-dimension scoring across after-tax IRR, operating cash flow, Year 1 ROI, \u00a7 469 eligibility, cumulative effective tax rate, total after-tax profit, NOI growth, and disposition tax concentration. Each dimension is scored favorable / solid / marginal / unfavorable with a one-line diagnostic.');
    } else {
    const verdictIcon = {
    ...
    scorecardHtml = '<div class="scorecard-panel"><div class="scorecard-panel-label">Scorecard</div>';
    v.scorecard.forEach(item => {
      scorecardHtml += '<div class="scorecard-row">' +
        '<div class="scorecard-icon">' + (verdictIcon[item.verdict] || verdictIcon.neutral) + '</div>' +
        '<div class="scorecard-label">' + item.label + '</div>' +
        '<div class="scorecard-note">' + item.note + '</div>' +
...
function renderForMode(mode) {
  ...
  renderSummary({
    ...
    cumEffRate: r.cumEffRate,                       // federal
    cumStateEffRate: r.cumStateEffRate,             // state + locality
    cumCombinedEffRate: r.cumCombinedEffRate,       // combined
    netFedTax: r.netFedTax,
    netSaleProceeds: r.netSaleProceeds,
    totalSaleTaxFed: r.totalSaleTaxFed,
    totalSaleTaxState: r.totalSaleTaxState,
    totalSaleTaxCombined: r.totalSaleTaxCombined,
    ...
    year1StateTaxSavings: c.year1StateTaxSavings,
  ...
  renderSaleAnalysis({
    ...
    stateSaleTax: c.stateSaleTax,             // raw state tax before exit-strategy
    localitySaleTax: c.localitySaleTax,       // raw locality tax before exit-strategy
    stateAndLocalitySaleTax: c.stateAndLocalitySaleTax,  // raw total
    effectiveStateSaleTax: r.effectiveStateSaleTax,           // after exit-strategy (0 if applied)
    effectiveLocalitySaleTax: r.effectiveLocalitySaleTax,     // after exit-strategy
    effectiveStateAndLocalitySaleTax: r.effectiveStateAndLocalitySaleTax,
    stateExitApplied: r.stateExitApplied,
    exitStrategyStateSavings: r.exitStrategyStateSavings,
    stateAdjBasis: c.stateAdjBasis,
    stateHasTax: c.stateHasTaxFlag,
    stateConforms: c.stateConforms,
    stateName: c.stateName,
    locality: c.locality,
    localityInfo: c.localityInfo,
    cumStateTaxSavings: c.cumStateTaxSavings
  });

  // Disposition Strategy Comparison panel — shows Sale vs 1031 vs Step-Up
  renderDispositionComparison();

  // Re-render projection so the disposition / total-net-cash-flow rows reflect
  // the active mode's net sale proceeds (which differ by recap / exit selection).
  renderProjectionTable(c.yearlyResults, c.stateHasTaxFlag);
}

function setStrategyMode(mode) {
  // Public-tier gate: the Donovan Legal Tax Strategy toggle is members-only.
  if (isPublic()) {
    alert('The Donovan Legal Tax Strategy toggle is available to Gold, Platinum, and Reserve members of Donovan Legal PLLC. Please contact the firm to learn about membership.');
    return;
  }
  // Existing federal recapture-allocation toggle. Argument 'mode' is
  // 'standard' or 'donovan' for back-compat with the existing button onclick.
  window.__strRecapStrategy = (mode === 'donovan');
  window.__strMode = mode;  // back-compat for any code that still reads __strMode
  renderForMode(resolveStrategyMode());
}

function setExitStrategyMode(mode) {
  if (isPublic()) {
    alert('Donovan Exit Planning is available to Gold, Platinum, and Reserve members of Donovan Legal PLLC. Please contact the firm to learn about membership.');
    return;
  }
  // New state-exit toggle (Donovan State Exit Strategy).
  // 'mode' is 'standard' or 'donovan'.
  window.__strExitStrategy = (mode === 'donovan');
  renderForMode(resolveStrategyMode());
}
...
function resetSTR() {
  ...
  // v2: reset new property type and month defaults
  document.getElementById('property_type').value = 'residential';
  document.getElementById('month_placed_in_service').value = '1';
  document.getElementById('month_disposition').value = '12';
...
function exportToCSV() {
  ...
  if (isPublic()) {
    alert('CSV export is available to Gold, Platinum, and Reserve members of Donovan Legal PLLC. Please sign in or contact the firm to learn about membership.');
    return;
  }
  const r = window.__strResults;
  ...
  lines.push(csvRow(['After-Tax IRR (federal + state)', csvPct(std.irr * 100), csvPct(dvn.irr * 100), csvPct((dvn.irr - std.irr) * 100)]));
  lines.push(csvRow(['Cumulative Effective Federal Tax Rate', csvPct(std.cumEffRate), csvPct(dvn.cumEffRate), csvPct(std.cumEffRate - dvn.cumEffRate) + ' (lower is better)']));
  if (c.stateHasTaxFlag) {
    lines.push(csvRow(['Cumulative Effective State + Locality Rate', csvPct(std.cumStateEffRate || 0), csvPct(dvn.cumStateEffRate || 0), csvPct((std.cumStateEffRate || 0) - (dvn.cumStateEffRate || 0)) + ' (lower is better)']));
    lines.push(csvRow(['Cumulative Effective Combined Rate', csvPct(std.cumCombinedEffRate || 0), csvPct(dvn.cumCombinedEffRate || 0), csvPct((std.cumCombinedEffRate || 0) - (dvn.cumCombinedEffRate || 0)) + ' (lower is better)']));
  }
  lines.push(csvRow(['Cash-on-Cash ROI', csvPct(std.roi), csvPct(dvn.roi), csvPct(dvn.roi - std.roi)]));
  lines.push(csvRow(['Year 1 Net Cash Benefit (operating CF + fed + state savings)', csvNum(c.year1NetCashBenefit), csvNum(c.year1NetCashBenefit), '0']));
  lines.push(csvRow(['Year 1 Federal Tax Savings', csvNum(c.year1FedTaxSavings), csvNum(c.year1FedTaxSavings), '0']));
  if (c.stateHasTaxFlag) {
    lines.push(csvRow(['Year 1 State + Locality Tax Savings', csvNum(c.year1StateTaxSavings || 0), csvNum(c.year1StateTaxSavings || 0), '0']));
  }
  lines.push(csvRow(['Cumulative Federal Operating Tax Savings', csvNum(c.cumFedTaxSavings), csvNum(c.cumFedTaxSavings), '0']));
  if (c.stateHasTaxFlag) {
    lines.push(csvRow(['Cumulative State + Locality Operating Tax Savings', csvNum(c.cumStateTaxSavings || 0), csvNum(c.cumStateTaxSavings || 0), '0']));
  }
  ...
  if (c.stateHasTaxFlag) {
    lines.push(csvRow(['State + Locality Tax on Sale', csvNum(std.totalSaleTaxState || 0), csvNum(dvn.totalSaleTaxState || 0), csvNum((std.totalSaleTaxState || 0) - (dvn.totalSaleTaxState || 0))]));
    lines.push(csvRow(['Combined Tax on Sale (fed + state + locality)', csvNum(std.totalSaleTaxCombined || 0), csvNum(dvn.totalSaleTaxCombined || 0), csvNum((std.totalSaleTaxCombined || 0) - (dvn.totalSaleTaxCombined || 0))]));
  }
  lines.push(csvRow(['Net Sale Proceeds (after federal + state tax)', csvNum(std.netSaleProceeds), csvNum(dvn.netSaleProceeds), csvNum(dvn.netSaleProceeds - std.netSaleProceeds)]));
  lines.push(csvRow(['Total After-Tax Profit (federal + state)', csvNum(std.totalProfit), csvNum(dvn.totalProfit), csvNum(dvn.totalProfit - std.totalProfit)]));
  lines.push('');

  // Multi-year projection (now with Cumulative column, acquisition outflow row, disposition inflow row)
  lines.push('=== MULTI-YEAR PROJECTION ===');
  const initInv         = c.initialInvestment || 0;
  const csvNetSaleProc  = std.netSaleProceeds || 0;          // export uses Standard mode for the projection
  const saleYr          = c.saleYear || yrs.length;
  const yrHeader = ['Item'].concat(yrs.map(y => 'Year ' + y.year)).concat(['Cumulative']);
  lines.push(csvRow(yrHeader));
  // Helper to add a row with sum across years as the cumulative cell
  function rowS(label, vals) {
    let sum = 0;
    vals.forEach(v => { if (typeof v === 'number' && isFinite(v)) sum += v; });
    lines.push(csvRow([label].concat(vals.map(csvNum)).concat([csvNum(sum)])));
  }
  function rowE(label, vals) {
    const last = vals.length > 0 ? vals[vals.length - 1] : 0;
    lines.push(csvRow([label].concat(vals.map(csvNum)).concat([csvNum(last)])));
  }
  rowS('Property Value (3% growth)',        yrs.map(y => y.propValue));
  rowS('NOI (3% growth)',                   yrs.map(y => y.noi));
  rowS('Mortgage Interest',                 yrs.map(y => y.interest));
  rowS('Mortgage Principal',                yrs.map(y => y.principal));
  rowS('Federal Depreciation',              yrs.map(y => y.fedDep));
  rowS('State Depreciation',                yrs.map(y => y.stateDep));
  rowS('Rental Net Income (Federal)',       yrs.map(y => y.rentalIncomeFed));
  rowS('Loss Usable Post-461(l)',           yrs.map(y => y.lossUsable461));
  rowS('NOL Generated',                     yrs.map(y => y.nolGenerated));
  rowS('NOL Used (80% rule)',               yrs.map(y => y.nolUsed));
  rowE('NOL Balance EOY',                   yrs.map(y => y.nolBalance));
  rowS('Federal Tax Impact (savings or owed)', yrs.map(y => y.fedTaxSavings));
  if (c.stateHasTaxFlag) {
    rowS('State + Locality Tax Impact',     yrs.map(y => y.stateTaxSavings));
  }
  rowS('Operating Cash Flow After Tax',     yrs.map(y => y.cashFlowAfterTax));
  rowE('Mortgage Balance EOY',              yrs.map(y => y.mortgageBalance));
  // Acquisition / disposition / total net cash flow — the IRR series
  rowS('Cash Outflow to Acquire (Yr 1)',    yrs.map(y => y.year === 1 ? -initInv : 0));
  rowS('Net Sale Proceeds at Disposition',  yrs.map(y => y.year === saleYr ? csvNetSaleProc : 0));
  rowS('Total Net Cash Flow (Acq + Ops + Disp)', yrs.map(y => (y.year === 1 ? -initInv : 0) + (y.cashFlowAfterTax || 0) + (y.year === saleYr ? csvNetSaleProc : 0)));
  lines.push('');
  ...
  if (c.stateHasTaxFlag) {
    lines.push(csvRow(['State Adjusted Basis', csvNum(c.stateAdjBasis)]));
    lines.push(csvRow(['State Gain (may differ from federal)', csvNum(c.stateGain)]));
    lines.push(csvRow(['State Tax on Sale', csvNum(c.stateSaleTax)]));
    if (c.localityInfo) {
      lines.push(csvRow([c.localityInfo.name + ' Locality Tax on Sale', csvNum(c.localitySaleTax || 0)]));
    }
    lines.push(csvRow(['Total State + Locality Tax on Sale', csvNum(c.stateAndLocalitySaleTax || 0)]));
    lines.push(csvRow(['Cumulative State + Locality Tax Savings (Operating Years)', csvNum(c.cumStateTaxSavings || 0)]));
    lines.push(csvRow(['Net State + Locality Tax Position Over Hold', csvNum((c.stateAndLocalitySaleTax || 0) - (c.cumStateTaxSavings || 0))]));
    lines.push(csvRow(['Combined Tax on Sale (Federal + State + Locality)', csvNum(std.totalSaleTaxCombined || 0)]));
  }
  lines.push(csvRow(['Mortgage Payoff', csvNum(c.mortgagePayoff)]));
  lines.push(csvRow(['Net Sale Proceeds (Standard, after federal + state tax)', csvNum(std.netSaleProceeds)]));
  lines.push('');
  ...
  if (c.stateHasTaxFlag) {
    lines.push(csvRow(['State + Locality Tax on Sale (Strategy)', csvNum(dvn.totalSaleTaxState || 0)]));
    lines.push(csvRow(['Combined Tax on Sale (Strategy)', csvNum(dvn.totalSaleTaxCombined || 0)]));
  }
  lines.push(csvRow(['Net Sale Proceeds (Strategy, after federal + state tax)', csvNum(dvn.netSaleProceeds)]));
  lines.push('');
...
// Wire the locality dropdown's change event to refresh the note panel.
// (Listener is attached after populateStates() because populateLocalitiesForState()
// runs as part of populateStates and creates the locality options.)
(function attachLocalityListener() {
  const locEl = document.getElementById('locality');
  if (locEl) {
    locEl.addEventListener('change', function() {
      if (typeof updateLocalityNote === 'function') updateLocalityNote();
    });
  }
})();
...
const STR_PRESETS = {
  ...
  corporate: {
    label: 'Corporate / Executive Medium-Term',
    rental_period: 'le30',
    sig_personal_services: 'yes',
    extra_personal_services: 'no',
    incidental_rental: 'no',
    business_hours_avail: 'no',
    jv_use: 'no',
  ...
  hotel: {
    ...
    ltv_band: '70',
    loan_type: 'conv',
    disposition_strategy: 'stepup',
    prop_type: 'multi'
  },
  incidental: {
    label: 'Incidental Rental — Exception (D)',
    rental_period: 'gt30',
    sig_personal_services: 'no',
    extra_personal_services: 'no',
    incidental_rental: 'yes',
    business_hours_avail: 'no',
    jv_use: 'no',
    reps: 'no',
    management: 'self',
  ...
  businesshours: {
    label: 'Business-Hours Facility — Exception (E)',
    rental_period: 'le7',
    sig_personal_services: 'no',
    extra_personal_services: 'no',
    incidental_rental: 'no',
    business_hours_avail: 'yes',
    jv_use: 'no',
    reps: 'no',
    management: 'self',
    taxpayer_hours: 600,
  ...
  selfrental: {
    label: 'Self-Rental to S-Corp — Exception (F)',
    rental_period: 'gt30',
    sig_personal_services: 'no',
    extra_personal_services: 'no',
    incidental_rental: 'no',
  ...
  reps: {
    label: 'REPS — Real Estate Professional',
    rental_period: 'gt30',
    sig_personal_services: 'no',
    extra_personal_services: 'no',
    incidental_rental: 'no',
  ...
  ltr: {
    label: 'LTR Baseline — Per Se Passive',
    rental_period: 'gt30',
    sig_personal_services: 'no',
    extra_personal_services: 'no',
    incidental_rental: 'no',
...
const STR_TRACKED_FIELDS = [
  ...
  'state', 'locality', 'hold_period', 'bonus_rate',
...
function restoreInputsFromLocalStorage() {
  try {
    ...
    let restored = 0;
    // First pass: restore everything except locality (locality depends on state being set first)
    STR_TRACKED_FIELDS.forEach(id => {
    ...
    // Now that state is restored, refresh the locality dropdown options for that state
    if (typeof populateLocalitiesForState === 'function') {
      populateLocalitiesForState();
    }
    // Second pass: restore locality (its options now exist)
    if (data.locality !== undefined) {
      const el = document.getElementById('locality');
      if (el) {
        // Only set if the saved value is a valid option for this state
        const opt = el.querySelector('option[value="' + data.locality + '"]');
        if (opt) { el.value = data.locality; restored++; }
      }
    }
    if (typeof updateLocalityNote === 'function') updateLocalityNote();
    // Restore NOI method
...
// Run on load: restore inputs, attach auto-save, and check for ?preset=X / #preset=X
// in the URL. The preset hash supports deep links from tools.html (and from anywhere
// else the firm wants to surface a specific scenario — blog posts, member emails).
function applyPresetFromUrl() {
  let key = null;
  // Check hash first (preferred — preserved on reload, doesn't show in history)
  ...
  // Fall back to query string for shareable links
  ...
  if (!key) return false;
  // Validate against the known preset keys to avoid silent failures on bad input
  const VALID_PRESETS = ['beach', 'ski', 'urban', 'corporate', 'hotel', 'incidental', 'businesshours', 'selfrental', 'reps', 'ltr'];
  if (VALID_PRESETS.indexOf(key) === -1) return false;
  try {
    loadPreset(key);
    return true;
  } catch (e) {
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', function() {
    ...
    attachAutoSave();
    // URL preset wins over restored inputs — a deep link from tools.html is a fresh intent
    const presetApplied = applyPresetFromUrl();
    if (presetApplied) {
      showToast('Loaded scenario from link');
    } else if (restored) {
      showToast('Restored your previously saved inputs');
    }
  });
} else {
...
// 2026-09-04: the analyzer page now carries its own scenario chips (tools.html had
// them; mobile readers landed on a wall of text instead). A chip is a hash link, so
// re-run the preset when the hash changes on the page already open.
window.addEventListener('hashchange', function () {
  try {
    if (applyPresetFromUrl()) {
      showToast('Loaded scenario');
      const first = document.getElementById('step_1');
      if (first && typeof first.scrollIntoView === 'function') first.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }
  } catch (e) { /* a bad hash is just ignored */ }
});
...
const STR_STEP_REQUIRED_FIELDS = {
  // Renumbered to follow real estate life cycle ordering (Batch C.4):
  //   1=Taxpayer, [Acquisition] 2=Property 3=Financing, [Ownership] 4=§469 5=MP 6=Ops 7=Hold, [Disposition] 8=Exit
  1: ['filing_status', 'state'],                          // Taxpayer
  2: ['purchase_price', 'land_pct', 'reclass_pct'],       // Property Information
  3: ['down_payment_pct', 'mortgage_rate'],               // Financing
  4: ['rental_period'],                                   // Per Se Passive — at minimum need average period
  5: ['taxpayer_hours'],                                  // Material Participation — at minimum need hours (unless REPS)
  6: ['cap_rate'],                                         // Operations (or gross_rents)
  7: ['hold_period', 'appreciation_rate'],                // Hold & Assumptions
  8: ['disposition_strategy', 'selling_costs_pct']        // Disposition Strategy & Selling Costs
};
...
function isStepComplete(stepNum) {
  if (stepNum === 4) {
    // Step 4 — Per Se Passive (§469): needs rental_period AND at least one exception path OR REPS
    ...
    // Step "complete" if there's a clear path:
  ...
  if (stepNum === 5) {
    // Step 5 — Material Participation: hours entered OR REPS path
  ...
  if (stepNum === 6) {
    // Step 6 — Operations: cap_rate OR (gross_rents + opex_ratio)
    const noiBtn = document.querySelector('[data-target="noi_method"].active');
...
renderProjectionTable = function(years, hasState) {
  ...
  tbl.querySelectorAll('tbody tr:not(.section-header)').forEach(tr => {
    ...
    for (let i = 1; i < cells.length; i++) {
      if (i === cells.length - 1 && years.length > 0 && cells.length === years.length + 2) {
        // Last cell is the Cumulative column (years.length year cells + 1 label cell + 1 cumulative = years.length + 2 total)
        cells[i].setAttribute('data-year-label', 'Cumulative:');
      } else {
...
function renderHeadlinePanel() {
  ...
  // The headline cards should reflect the ACTIVE mode (recap / exit / both) so they
  // move when the user toggles either strategy — not stay frozen on Standard.
  const modeKey = (typeof resolveStrategyMode === 'function') ? resolveStrategyMode() : 'none';
  const active  = r[modeKey] || std;

  // Year 1 Net Cash Benefit — already combined (operating CF + fed savings + state savings)
  const yr1Benefit  = c.year1NetCashBenefit;
  const yr1Fed      = c.year1FedTaxSavings   || 0;
  const yr1State    = c.year1StateTaxSavings || 0;

  // Cumulative operating tax savings — break out fed and state, sum to combined
  const cumFedSavings      = c.cumFedTaxSavings   || 0;
  const cumStateSavings    = c.cumStateTaxSavings || 0;
  const cumCombinedSavings = cumFedSavings + cumStateSavings;

  // Disposition tax exposure — show the ACTIVE mode's sale tax (combined fed + state +
  // locality). When DLTS is on, federal drops; when Exit Planning is on, state drops;
  // under §1031 / Step-Up, both go to zero. Standard counterfactual is shown for delta.
  const dispoTaxFed      = active.totalSaleTaxFed      || 0;
  const dispoTaxState    = active.totalSaleTaxState    || 0;
  const dispoTaxCombined = dispoTaxFed + dispoTaxState;
  const stdDispoCombined = (std.totalSaleTaxFed || 0) + (std.totalSaleTaxState || 0);
  const strategySavings  = stdDispoCombined - dispoTaxCombined;  // total savings vs Standard (DLTS+Exit+disposition strategy all flow through here)
    ...
    <div class="headline-panel">
      <div class="headline-grid">
        <div class="headline-card">
          ...
          <div class="headline-sub">Operating cash flow + federal + state tax savings, Year 1</div>
          <div class="headline-sub" style="margin-top:0.2rem; font-size:0.72rem;">Fed savings ${fmt(yr1Fed)} &middot; State + locality ${fmt(yr1State)}</div>
        </div>
        <div class="headline-card">
          <div class="headline-label">Cumulative Operating Tax Savings</div>
          <div class="headline-value">${fmt(cumCombinedSavings)}</div>
          <div class="headline-sub">Federal + state tax savings across the ${c.saleYear}-year hold from operating losses</div>
          <div class="headline-sub" style="margin-top:0.2rem; font-size:0.72rem;">Federal ${fmt(cumFedSavings)} &middot; State + locality ${fmt(cumStateSavings)}</div>
        </div>
        <div class="headline-card">
          <div class="headline-label">Disposition Tax Exposure</div>
          <div class="headline-value">${fmt(dispoTaxCombined)}</div>
          <div class="headline-sub">Combined fed + state + locality sale tax under the active strategy${strategySavings > 0 ? '; total savings vs Standard ' + fmt(strategySavings) : ''}</div>
          <div class="headline-sub" style="margin-top:0.2rem; font-size:0.72rem;">Federal ${fmt(dispoTaxFed)} &middot; State + locality ${fmt(dispoTaxState)}</div>
        </div>
      </div>
...
function computeStrategyComparison() {
  ...
  try {
    ...
    if (ltrInputs._ltr_passive) {
      ...
      const grossSaleStd = ltrResult.common.netSalePrice - ltrResult.common.mortgagePayoff - ltrResult.standard.totalSaleTaxFed - (ltrResult.common.stateAndLocalitySaleTax || ltrResult.common.stateSaleTax || 0);
...
function renderStrategyComparison() {
  ...
  if (!panel) return;
  // Public-tier gate: STR vs LTR vs Hybrid comparison shows the operating-strategy
  // decision tree. Lock with a structural teaser in public mode.
  if (isPublic()) {
    const rows = ['Year 1 NOI','Year 1 Federal Tax Savings','Year 1 State + Locality Tax Savings','Year 1 Net Cash Benefit','Cum. Federal Operating Tax Savings','Cum. State + Locality Operating Tax Savings','Cum. Combined Operating Tax Savings','Net Sale Proceeds (Std)','Total After-Tax Profit','Profit as % of Capital','After-Tax IRR'];
    const placeholder = `
      <div class="strategy-comparison-panel">
        ...
        <div class="strategy-comparison-table-wrapper">
          <table class="strategy-comparison-table">
            <thead><tr><th></th><th>STR</th><th>LTR</th><th>Hybrid</th></tr></thead>
            <tbody>${rows.map(r => `<tr><td class="sc-label">${r}</td><td class="sc-value">\u2588\u2588\u2588,\u2588\u2588\u2588</td><td class="sc-value">\u2588\u2588\u2588,\u2588\u2588\u2588</td><td class="sc-value">\u2588\u2588\u2588,\u2588\u2588\u2588</td></tr>`).join('')}</tbody>
          </table>
        </div>
      </div>`;
    panel.innerHTML = tierLock(placeholder, 'Strategy Comparison \u2014 Member access', 'Side-by-side modeling of three operating strategies: short-term rental with material participation (\u00a7 1.469-1T(e)(3)(ii)), long-term rental with passive activity treatment, and the Hybrid that claims STR Year 1 only then converts to LTR. Shows after-tax IRR, total profit, and full federal + state tax savings for each path.');
    return;
  }
    ...
    <div class="strategy-comparison-panel">
      ...
      <table class="strategy-comparison-table">
        ...
        <tbody>
          ...
          ${row('Year 1 Federal Tax Savings', str.common.year1FedTaxSavings, ltr ? ltr.common.year1FedTaxSavings : null, hybrid ? hybrid.common.year1FedTaxSavings : null)}
          ${row('Year 1 State + Locality Tax Savings', str.common.year1StateTaxSavings || 0, ltr ? (ltr.common.year1StateTaxSavings || 0) : null, hybrid ? (hybrid.common.year1StateTaxSavings || 0) : null)}
          ${row('Year 1 Net Cash Benefit (after fed + state)', str.common.year1NetCashBenefit, ltr ? ltr.common.year1NetCashBenefit : null, hybrid ? hybrid.common.year1NetCashBenefit : null)}
          ${row('Cum. Federal Operating Tax Savings', str.common.cumFedTaxSavings, ltr ? ltr.common.cumFedTaxSavings : null, hybrid ? hybrid.common.cumFedTaxSavings : null)}
          ${row('Cum. State + Locality Operating Tax Savings', str.common.cumStateTaxSavings || 0, ltr ? (ltr.common.cumStateTaxSavings || 0) : null, hybrid ? (hybrid.common.cumStateTaxSavings || 0) : null)}
          ${row('Cum. Combined Operating Tax Savings', (str.common.cumFedTaxSavings || 0) + (str.common.cumStateTaxSavings || 0), ltr ? ((ltr.common.cumFedTaxSavings || 0) + (ltr.common.cumStateTaxSavings || 0)) : null, hybrid ? ((hybrid.common.cumFedTaxSavings || 0) + (hybrid.common.cumStateTaxSavings || 0)) : null)}
          ${row('Net Sale Proceeds (Std, after fed + state)', str.standard.netSaleProceeds, ltr ? ltr.standard.netSaleProceeds : null, hybrid ? hybrid.standard.netSaleProceeds : null)}
          ${row('Total After-Tax Profit (after fed + state)', str.standard.totalProfit, ltr ? ltr.standard.totalProfit : null, hybrid ? hybrid.standard.totalProfit : null)}
...
// 2026-09-05 mobile pass: the notice and the progress grid are <details>.
// Phones arrive with them closed (the chips and Step 1 are the first thing
// the visitor sees); on wider screens they open so nothing changes there.
(function () {
  const wide = window.matchMedia && window.matchMedia('(min-width: 769px)').matches;
  document.querySelectorAll('details.dl-mobile-collapse').forEach(function (d) { d.open = wide; });
})();
